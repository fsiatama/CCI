...
(function(){
	...
	var colModelIndicador = new Ext.grid.ColumnModel({
		columns:[
			...
			{header:'<?= Lang::get('indicador.columns_title.CA'); ?>', dataIndex:'CA','renderer':unsigned4Format},
			{header:'<?= Lang::get('indicador.columns_title.CAA'); ?>', dataIndex:'CAA','renderer':unsigned4Format},
		]
		,defaults: {
			sortable: true
			,align: 'right'

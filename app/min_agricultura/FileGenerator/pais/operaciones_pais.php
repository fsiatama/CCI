<?php
session_start();
include('../../lib/config.php');
include_once(PATH_APP."lib/idioma.php");
include_once(PATH_APP."lib/lib_funciones.php");
include_once(PATH_APP."lib/lib_sesion.php");
include_once(PATH_RAIZ."min_agricultura/lib/pais/paisAdo.php");
$paisAdo = new PaisAdo("min_agricultura");
$pais    = new Pais;
if(isset($accion)){
	switch($accion){
		case "act":
			$pais->setId_pais($id_pais);
			$pais->setPais($pais);
			$rs_pais = $paisAdo->actualizar($pais);
			if($rs_pais !== true){
				$success = false;
			}
			else{
				$success = true;
			}
			$respuesta = array(
				"success"=>$success,
				"errors"=>array("reason"=>$rs_pais)
			);
			echo json_encode($respuesta);
			exit();
		break;
		case "del":
			$pais->setId_pais($id_pais);
			$rs_pais = $paisAdo->borrar($pais);
			if($rs_pais !== true){
				$success = false;
			}
			else{
				$success = true;
			}
			$respuesta = array(
				"success"=>$success,
				"errors"=>array("reason"=>$rs_pais)
			);
			echo json_encode($respuesta);
			exit();
		break;
		case "crea":
			$pais->setId_pais($id_pais);
			$pais->setPais($pais);
			$rs_pais = $paisAdo->insertar($pais);
			if($rs_pais["success"] !== true){
				$respuesta = array(
					"success"=>false,
					"errors"=>array("reason"=>"Error creando pais", "error"=>$rs_pais["error"])
				);
				echo json_encode($respuesta);
				exit();
			}
			$id_pais = $rs_pais["insert_id"];
			$respuesta = array(
				"success"=>true,
				"errors"=>array("reason"=>$id_pais)
			);
			echo json_encode($respuesta);
			exit();
		break;
		case "lista":
			$arr = array();
			$pais->setId_pais($id_pais);
			$pais->setPais($pais);
			$rs_pais = $paisAdo->lista($pais);
			if(!is_array($rs_pais)){
				$respuesta = array(
					"success"=>false,
					"errors"=>array("reason"=>$rs_pais)
				);
				echo json_encode($respuesta);
				exit();
			}
			foreach($rs_pais["data"] as $key => $data){
				$arr[] = sanear_string($data);
			}
			$respuesta = array(
				"success"=>true,
				"total"=>$rs_pais["total"],
				"data"=>$arr
			);
			echo json_encode($respuesta);
			exit();
		break;
		case "lista_filtro":
			$arr = array();
			$start = (isset($start))?$start:0;
			$limit = (isset($limit))?$limit:MAXREGEXCEL;
			$page = ($start==0)?1:($start/$limit)+1;
			$limit = $page . ", " . $limit;
			$rs_pais = $paisAdo->lista_filtro($query, $valuesqry, $limit);
			if(!is_array($rs_pais)){
				$respuesta = array(
					"success"=>false,
					"errors"=>array("reason"=>$rs_pais)
				);
				echo json_encode($respuesta);
				exit();
			}
			elseif($rs_pais["total"] == 0){
				$respuesta = array(
					"success"=>false,
					"errors"=>array("reason"=>sanear_string(_NOSEENCONTRARONREGISTROS))
				);
				echo json_encode($respuesta);
				exit();
			}
			else{
				foreach($rs_pais["data"] as $key => $data){
					$arr[] = sanear_string($data);
				}
				$respuesta = array(
					"success"=>true,
					"total"=>$rs_pais["total"],
					"data"=>$arr
				);
				echo json_encode($respuesta);
				exit();
			}
		break;
	}
}
?>

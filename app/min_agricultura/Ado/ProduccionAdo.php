...
class ProduccionAdo extends BaseAdo {
	...
	public function buildSelect()
	{
		...
		$sql = 'SELECT
			 ...
			 produccion_uupdate,
			 sector_nombre
			FROM produccion LEFT JOIN sector ON produccion_sector_id = sector_id
		';
		$sql .= $this->buildSelectWhere();

		$sql .= ' ORDER BY produccion_anio';
		...
		foreach($this->data as $key => $data){
			if ($data <> ''){
				...
				elseif ($operator == 'IN') {
					...
					//solo para busquedas LIKE realiza la busqueda textual en la tabla auxiliar
					if ($key == 'produccion_sector_id') {
						$key = 'sector_nombre';
					}
					$filter[] = $key . ' ' . $operator . ' "%' . $data . '%"';
					$joinOperator = ' OR ';

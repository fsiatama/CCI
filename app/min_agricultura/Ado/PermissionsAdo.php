...
class PermissionsAdo extends BaseAdo {
	...
	public function buildSelect()
	{
		...
		$sql = 'SELECT
			 ...
			 category_menu_order
			FROM permissions 
			LEFT JOIN menu ON permissions_menu_id = menu_id
			LEFT JOIN category_menu ON menu_category_menu_id = category_menu_id
		';
		if(!empty($filter)){
			$sql .= ' WHERE ('. implode( $joinOperator, $filter ).')';
		}

		$sql .= ' ORDER BY category_menu_order, menu_order';

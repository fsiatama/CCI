...
*
* MarimekkoChart chart extends the SingleYAxis2DVerticalChart class to render a
* Marimekko Chart.
*/
//Import Chart class
...
import com.fusioncharts.core.chartobjects.Column2DNoRightBorder;
//Legend Class
...
class com.fusioncharts.core.charts.MarimekkoChart extends SingleYAxis2DVerticalChart
{
	...
	//Whether all indexes sum to 100. Makes it a 100% chart.
	private var allSum100:Boolean;	
	//Grand sum
	private var grandSum:Number;
	...
	*/
	function MarimekkoChart (targetMC : MovieClip, depth : Number, width : Number, height : Number, x : Number, y : Number, debugMode : Boolean, lang : String, scaleMode:String, registerWithJS:Boolean, DOMId:String)
	{
		...
		this.log ("Chart Type", "Marimekko Column Chart", Logger.LEVEL.INFO);
		_version = getFCVersion();
		this.log ("Version", _version, Logger.LEVEL.INFO);
		//List Chart Objects and set them
		_arrObjects = new Array ("BACKGROUND", "CANVAS", "CAPTION", "SUBCAPTION", "YAXISNAME", "XAXISNAME", "DIVLINES", "YAXISVALUES", "HGRID", "DATALABELS", "DATAVALUES", "TRENDLINES", "TRENDVALUES", "DATAPLOT", "TOOLTIP", "VLINES", "LEGEND", "VLINELABELS", "XAXISPERCENTVALUES");
	...
	*/
	public function render (isRedraw:Boolean) : Void
	{
		...
		} else
		{
			...
			//Draw x-axis percent values
			this.config.intervals.xAxisPercent = setInterval (Delegate.create (this, drawXAxisPercentValues) , this.timeElapsed);
			//Update timer
			this.timeElapsed += (this.params.animation) ? this.styleM.getMaxAnimationTime (this.objects.TRENDLINES, this.objects.TRENDVALUES, this.objects.VLINES, this.objects.DATAVALUES, this.objects.XAXISPERCENTVALUES) : 0;
	...
	*	@param	widthPercent	Width percent of this category
	...
	*/
	private function returnDataAsCat (label : String, widthPercent:Number, showLabel : Number, toolText : String) : Object
	{
		//Create container object
		var catObj : Object = new Object ();
		catObj.label = label;
		catObj.widthPercent = widthPercent;
					...
					if (arrLevel1Nodes [j].nodeName.toUpperCase () == "CATEGORIES")
					{
						...
						for (k = 0; k < arrLevel2Nodes.length; k ++)
						{
							if (arrLevel2Nodes [k].nodeName.toUpperCase () == "CATEGORY")
							{
								...
								//Percent width of this category
								var catWidthPercent = categoryAtt ["widthpercent"];
								//Store it in data container.
								this.categories [this.num] = this.returnDataAsCat (catLabel, catWidthPercent, catShowLabel, catToolText);
							} 
					...
					} else if (arrLevel1Nodes [j].nodeName.toUpperCase () == "DATASET")
					{
						...
						for (k = 0; k < arrLevel2Nodes.length; k ++)
						{
							if (arrLevel2Nodes [k].nodeName.toUpperCase () == "SET")
							{
								...
								//Now, get value. Convert all to positive for this chart.
	...
	*/
	private function parseAttributes (graphElement : XMLNode) : Void
	{
		...
		//emanates from 0 only when usePercentDistribution value  is true .
		...
		//Whether percent values are to be used
		this.params.usePercentDistribution = toBoolean (getFN (atts ["usepercentdistribution"] , 1));
		...
		//Configuration whether to show x-axis percent values
		this.params.showXAxisPercentValues = toBoolean (getFN (atts ["showxaxispercentvalues"] , 1));
	...
	* index in all the data sets.
	*	@param	index	Index for which we've to calculate sum
	*	@return			Sum of all the data items of all datasets for the specified index.
	*/
	private function getSumOfValues (index : Number) : Number
	{
		//Loop variables
		var i : Number;
		...
		for (i = 1; i <= this.numDS; i ++)
		{
			...
			{
				sum += this.dataset [i].data [index].value;
			}
		}
		if (this.sumStored == false)
		{
			...
			this.sums [index].sum = sum;
			grandSum += sum;
			//Update allSum100 flag
			if (Math.round(sum)!=100){
				this.allSum100 = false;
			}
			//Update flag
			if (index == this.num)
			{
	...
	*/
	private function getMaxDataValue () : Number
	{
		...
		{
			vitalSum = this.getSumOfValues (i);
	...
	*/
	private function preparePercentData(): Void{
		if (this.params.usePercentDistribution){
			var i:Number;
			var j:Number;
			...
			//If each set already adds up to 100, no need to do anything
			if (this.allSum100){
				//Over-ride numbersuffix and return
			...
			for (i = 1; i <= this.numDS; i ++)
			{
				for (j = 1; j <= this.num; j ++)
				{
					if (this.dataset [i].data [j].isDefined){
						//Set it's display value as the formatted actual value 
						...
						//Now convert it's value to %, based on its index
						this.dataset [i].data [j].value = (this.dataset [i].data [j].value/this.sums[j].sum)*100;
			...
			//We do not calculate widthPercent here, as it's automatically calculated later.
			//The sums[] array stores the sums of original values. So, the widthPercent is
			//calculated according to the same.
			//Over-ride numberprefix, numbersuffix
			this.params.numberPrefix = "";
			this.params.numberSuffix = "%";
			//Set flag that the data now sums to 100%
			this.allSum100 = true;
		}
	}
	/**
	...
	*/
	...
	{
		//First, if all sums are 100, force the y-axis to be 100
		if (this.allSum100==true && this.params.usePercentDistribution){
	...
	*/
	private function setStyleDefaults () : Void
	{
		...
		delete trendFont;
		//-----------------------------------------------------------------//
		//Default font object for x-axis percent values
		//-----------------------------------------------------------------//
		var xAxisPercentValuesFont = new StyleObject ();
		xAxisPercentValuesFont.name = "_SdXAxisPercentValuesFont";
		xAxisPercentValuesFont.align = "center";
		xAxisPercentValuesFont.valign = "top";
		xAxisPercentValuesFont.font = this.params.baseFont;
		xAxisPercentValuesFont.size = this.params.baseFontSize - 1;
		xAxisPercentValuesFont.color = this.params.baseFontColor;
		xAxisPercentValuesFont.bgcolor = this.params.canvasBgColor;
		xAxisPercentValuesFont.bordercolor = this.params.canvasBorderColor;
		//Over-ride
		this.styleM.overrideStyle (this.objects.XAXISPERCENTVALUES, xAxisPercentValuesFont, this.styleM.TYPE.FONT, null);
		delete xAxisPercentValuesFont;
		//-----------------------------------------------------------------//
	...
	*/
	private function calculatePoints (isRedraw:Boolean)
	{
		...
		//Reserve space for showing sum of data.
		...
		//-------- Calculate width percent of each index ------------//
		//Before calculating column width, calculate the width of each block of column
		var sumCatWidth:Number = 0;
		for (i=1; i<=this.num; i++){
			//Check for existence and validity of category width
			if (this.categories[i].widthPercent!=undefined && isNaN(Number(this.categories[i].widthPercent))==false){
				//Convert to number
				this.categories[i].widthPercent = Number(this.categories[i].widthPercent);
			}else{
				//Allot width to the same, based on total width
				this.categories[i].widthPercent = (this.sums[i].sum / this.grandSum)*100;				
			}
			//Store total sum
			sumCatWidth += this.categories[i].widthPercent;
		}
		//If all categories width percent sum, does not equal 100, then reset them internally based on sum
		if (Math.round(sumCatWidth)!=100){
			for (i=1; i<=this.num; i++){
					//Allot width to the same, based on total width
					this.categories[i].widthPercent = (this.sums[i].sum / this.grandSum)*100;				
			}
		...
		{
			//Case 2 (WRAP): Create all the labels on the chart. 
			...
			for (i = 1; i <= this.num; i ++)
			{
				...
				if (this.categories [i].showLabel)
				{
					//Label width that can be alloted to it (max). Based on % width of this column
					var labelWidthAllocation:Number = (this.categories[i].widthPercent/100)*canvasWidth;
					//Store for this category
					this.categories[i].labelWidth = labelWidthAllocation;
					//Create text box and get height
					labelObj = createText (true, this.categories [i].label, this.tfTestMC, 1, testTFX, testTFY, 0, labelStyleObj, true, labelWidthAllocation, maxLabelHeight);
					//Store the larger
					this.config.maxLabelHeight = (labelObj.height > this.config.maxLabelHeight) ? (labelObj.height) : (this.config.maxLabelHeight);
				}
			}
		...
		if (this.params.showLegend)
		{
			...
			for (i = 1; i <= this.numDS; i ++)
			{
				...
				if (this.dataset [j].includeInLegend && this.dataset [j].seriesName != "")
				{
					...
					//Create the 2 icons
					var objIcons:Object = LegendIconGenerator.getIcons(LegendIconGenerator.RECTANGLE, iconHeight, true, objIcon);
		...
		//Now, all the categories have width percent properly defined either in terms of sum
		//or correct user input.
		this.config.columnWidth = this.elements.canvas.w / this.num;
		var columnX:Number = this.elements.canvas.x;
		var columnWidth:Number;
		var dataEndY : Number;
		//Now, store the positions of the columns
		for (i = 1; i <= this.num; i ++)
		{
			//Calculate column width for this column
			columnWidth = (this.categories[i].widthPercent/100)*this.elements.canvas.w;
			//Add half of column for center alignment
			columnX = columnX + columnWidth/2;
			//Position co-ordinates
			var posY : Number = 0;
			//Categories x-Position
			//this.categories [i].x = this.elements.canvas.x + this.config.columnWidth * (i - 0.5);
			this.categories [i].x = columnX;
			//Add the balance of column
			columnX = columnX + columnWidth/2;
			//Store toX
			this.categories [i].toX = columnX;
	...
	*/
	private function allotDepths () : Void
	{
		...
		//X-axis percent values
		this.dm.reserveDepths ("XAXISPERCENTVALUES", this.num);
		//Canvas Border
		...
		for (i = 1; i <= this.num; i ++)
		{
			...
			if (this.categories [i].showLabel)
			{
				...
				} else if (this.params.labelDisplay == "WRAP")
				{
					...
					labelStyleObj.align = "center";
					labelStyleObj.vAlign = "bottom";
					labelObj = createText (false, this.categories [i].label, this.cMC, depth, this.categories [i].x, this.elements.canvas.toY + this.params.labelPadding, 0, labelStyleObj, true, this.categories[i].labelWidth, this.config.wrapLabelHeight);
		...
		for (i = 1; i <= this.num; i ++)
		{
			...
			for (j = 1; j <= this.numDS; j ++)
			{
				...
				if (this.dataset [j].data [i].isDefined)
				{
					...
					if (allSum100){					
						//If it's a 100% chart, we create columns without right border so as to
						//avoid semi-pixel overlapping (thereby yielding thick lines)
						var colIns : Column2DNoRightBorder = new Column2DNoRightBorder (colMC, this.dataset [j].data [i].w, this.dataset [j].data [i].h, this.params.plotBorderColor, this.params.plotBorderAlpha, this.params.plotBorderThickness, this.dataset [j].data [i].color, this.dataset [j].data [i].alpha, this.dataset [j].data [i].ratio, this.params.plotFillAngle, true, this.dataset [j].data [i].dashed, this.params.plotBorderDashLen, this.params.plotBorderDashGap);
					}else{
						//Draw normal Column2D
	...
	*/
		...
		for (i = 1; i <= this.numDS; i ++)
		{
			for (j = 1; j <= this.num; j ++)
			{
				...
				if (this.dataset [i].data [j].isDefined && this.dataset [i].data [j].showValue)
				{
					...
					//If the height of text field is not fitting in data, remove
					if (valueObj.tf._height > Math.abs(this.dataset [i].data [j].h)){
	...
	/**
	 * Draws the x-axis percent values
	*/
	private function drawXAxisPercentValues():Void{
		//Draw, only if required.
		if (this.params.showXAxisPercentValues){
			//Get value text style
			var valueStyleObj : Object = this.styleM.getTextStyle (this.objects.XAXISPERCENTVALUES);
			//Container object
			var valueObj : MovieClip;
			//Depth
			var depth : Number = this.dm.getDepth ("XAXISPERCENTVALUES");
			//Iterate through all indexes
			var i:Number, cumSum:Number=0, displayValue:String;
			for (i = 1; i < this.num; i ++)
			{
				//Calculate the cumulative sum
				cumSum = cumSum+ this.categories[i].widthPercent;
				//When cumulative sum is NaN, skip
				if(!isNaN(cumSum)){
					//Calculate the display value
					displayValue = formatNumber (cumSum, this.params.formatNumber, this.params.decimals, this.params.forceDecimals, false, this.params.defaultNumberScale, this.config.nsv, this.config.nsu, "", "%", this.params.scaleRecursively, this.params.scaleSeparator, this.params.maxScaleRecursion);
					//Create the label
					valueObj = createText (false, displayValue, this.cMC, depth, this.categories [i].toX, this.elements.canvas.toY, 0, valueStyleObj, false, 0, 0);
					//Apply filter
					this.styleM.applyFilters (valueObj.tf, this.objects.XAXISPERCENTVALUES);
					//Apply animation
					if (this.params.animation)
					{
						this.styleM.applyAnimation (valueObj.tf, this.objects.XAXISPERCENTVALUES, this.macro, valueObj.tf._x, 0, valueObj.tf._y, 0, 100, null, null, null);
					}
					//Increase depth
					depth ++;
				}
		...
		//Clear interval
		clearInterval (this.config.intervals.xAxisPercent);
	}
	/**
		...
		if (this.params.showSum)
		{
			...
			for (i = 1; i <= this.num; i ++)
			{
				...
				if (this.sums [i].sum != 0)
				{
					//Get the y position
					vAlign = "top";
					yPos = this.config.basePlanePos + this.sums [i].pY;
					//Align position
					align = "center";
					//Convey alignment to rendering object
					valueStyleObj.align = align;

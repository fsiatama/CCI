	...
	*/
	private function drawTrendLines () : Void 
	{
		...
		for (i = 1; i <= this.numTrendLines; i ++)
		{
			...
			if(this.trendLines [i].isTrendZone && Math.abs(this.trendLines [i].w) < 0.3){
			...
			} else if (this.divLines [i].value == 0)
			{
				...
				if (this.params.showZeroPlane)
				{
					...
					if (this.params.divLineIsDashed)
					{
						...
						//Dashed line
						DrawingExt.dashTo (zeroPlaneMC, 0, - this.elements.canvas.h / 2, 0, this.elements.canvas.h / 2, len, gap);
					} else 
					{

...
class com.fusioncharts.components.FCChartHScrollBar extends MovieClip implements com.fusioncharts.components.IComponent {
	...
	//Extra shifts for mask (for covering portions not covered in canvas)
	private var maskXShift:Number, maskYShift:Number;
	//Constants
	 ...
	 *	@param	maskXShift			X-Shift for mask (internal shifting of mask for greater content area).
	 *								Note: Mask does NOT automatically increase paneWidth/paneHeight.
	 *	@param	maskYShift			Y-Shift for mask (internal shifting of mask for greater content area)
	 *	 							Note: Mask does NOT automatically increase paneWidth/paneHeight.
	 */
	function FCChartHScrollBar(contentMC:MovieClip, refMaskMC:MovieClip, scrollContainerMC:MovieClip, maskMC:MovieClip, paneWidth:Number, paneHeight:Number, scrollWidth:Number, scrollHeight:Number, btnWidth:Number, btnPadding:Number, maskXShift:Number, maskYShift:Number) {
		...
		//Mask shifts
		this.maskXShift = maskXShift;
		this.maskYShift = maskYShift;
	...
	*/
	 ...
	 * of scroll bar.
	 * @param		contentWidth 	At the time of reset scrollbar the contentwidth will used. 
	*/
	private function calculate(contentWidth:Number) {
		//Store Content width
		this.contentWidth = (isNaN(contentWidth))? this.refMaskMC._width : contentWidth;
	...
	*/
	private function drawMask(isRedraw:Boolean):Void {
		if(!isRedraw){
			//Clear the mask movie clip
			this.maskMC.clear();
			//Draw a rectangle within this movie clip
			this.maskMC.beginFill(0xffffff, 50);
			...
			this.maskMC._x = contentStartXPosition-this.maskXShift;
			this.maskMC._y = this.contentMC._y-this.maskYShift;
		}
		//Set the mask
	...
	*/
	...
	/**
	  * resetScrollProp method resets the scrollbar size and position based on dispaly area of canvas.
	  * @param 		percentOfTimelineDisplayed		Percentage of timeline displayed in the visible area.
	  * @param		contentStartPosition			Content start position based on which the scrollbar is reset.
	  */
	public function resetScrollProp(percentOfTimelineDisplayed:Number, contentStartPosition:Number):Void{
		//Store the previous color
		var scrollClr:String = this.scrollColor;
		//validate color 
		var defClr:String  = (scrollClr == "CCCCCC")? "DDDDDD" : "CCCCCC";
		var timeLineTotalWidth:Number =  Math.floor(this.paneWidth* (1/percentOfTimelineDisplayed));
		var contentStartPos = this.contentStartXPosition - Math.floor(timeLineTotalWidth*contentStartPosition) ;
		this.contentMC._x = this.contentStartXPosition;
		//Calculate the various dimensions required.
		this.calculate(timeLineTotalWidth);
		//Create children movie clips
		this.createChildren();
		//Set a default grey color shade
		this.setColor(defClr);
		//Draw the elements initially.
		//Track, face and buttons have been rendered by setColor method.
		this.drawMask(true);
		this.setEvents();
		...
		//Position the content mc
		this.contentMC._x = contentStartPos;
		//Update the scroll face position accordingly.
		//We adjust for the deducted face width (for width <15 pixels)
		var fX:Number = ((-(this.contentMC._x-this.contentStartXPosition)/timeLineTotalWidth)*this.trackWidth)+this.scrollStartXPosition-(this.faceAdjWidth*(-(this.contentMC._x-this.contentStartXPosition)/timeLineTotalWidth));
		//Update face and face click movie clips X position
		this.faceMC._x = fX;
		this.faceClickMC._x = fX;
		//Set the color
		this.setColor(scrollClr);
	}
}

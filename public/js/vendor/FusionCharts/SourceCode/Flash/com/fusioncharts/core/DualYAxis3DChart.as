...
class com.fusioncharts.core.DualYAxis3DChart extends DualYAxisChart 
{
	...
	private function drawTrendLines () : Void 
	{
		...
		for (i = 1; i <= this.numTrendLines; i ++)
		{
			...
			if (this.trendLines [i].isValid == true)
			{
				...
				{
					//Value to be placed (w.r.t position of parent-y axis)
					trendFontObj.align = (this.params.primaryAxisOnLeft==true)?"right":"left";
					//Calculate x-pos
					xPos = (this.params.primaryAxisOnLeft==true)?(this.elements.canvasBg.x - this.params.yAxisValuesPadding):(this.elements.canvas.toX + this.params.yAxisValuesPadding);
					//Create text
					trendValueObj = createText (false, this.trendLines [i].displayValue, this.cMC, valueDepth, xPos, this.trendLines [i].tbY, 0, trendFontObj, false, 0, 0);
					//X-position for text box animation
					tbAnimX = this.elements.canvasBg.x - this.params.yAxisValuesPadding - trendValueObj.width;
				} else 
				{
					//Left side
					trendFontObj.align = (this.params.primaryAxisOnLeft==true)?"left":"right";
					//Calculate x-position
					xPos = (this.params.primaryAxisOnLeft==false)?(this.elements.canvasBg.x - this.params.yAxisValuesPadding):(this.elements.canvas.toX + this.params.yAxisValuesPadding);
					//Create text
					trendValueObj = createText (false, this.trendLines [i].displayValue, this.cMC, valueDepth, xPos, this.trendLines [i].tbY, 0, trendFontObj, false, 0, 0);
					//X-position for text box animation
	...
	*/
	private function drawDivLines () : Void 
	{
		...
		//Set alignment
		divLineFontObj.align = (this.params.primaryAxisOnLeft==true)?"right":"left";
		...
		{			
			//When showPZeroPlaneValue is undefined reset as per inheritance. 
			if( this.divLines [i].value == 0){
				if(this.params.showPZeroPlaneValue == undefined){
					if(this.divLines [i].value ==  this.config.PYMin || this.divLines [i].value ==  this.config.PYMax ){
						this.params.showPZeroPlaneValue = this.params.showLimits;
					}else{
						this.params.showPZeroPlaneValue = this.params.showDivLineValues;
					}
				}
				this.divLines [i].showValue = this.params.showPZeroPlaneValue;
			}
			...
			else if ((i == 0) || (i == this.divLines.length - 1))
			{
				...
				if ((this.params.showPZeroPlaneValue && this.divLines [i].value == 0) || (this.params.showLimits && this.divLines [i].showValue))
				{
					depth ++;
					...
					divLineValueObj = createText (false, this.divLines [i].displayValue, this.cMC, depth, xPos, yPos, 0, divLineFontObj, false, 0, 0);
				}
			} else 
			{
		...
		divLineFontObj.align = (this.params.primaryAxisOnLeft==true)?"left":"right";
		//Flag whether to show the secondary div line value
		var showSDivValue : Boolean = false;
		//Calculate the x-position
		xPos = (this.params.primaryAxisOnLeft==true)?(this.elements.canvas.toX + this.params.yAxisValuesPadding):(this.elements.canvasBg.x - this.params.yAxisValuesPadding);
		//Sort the secondary divline array 
		this.sDivLines.sortOn("value",Array.NUMERIC);
		...
		for (i = 0; i < this.sDivLines.length; i ++)
		{
			...
			//When showSZeroPlaneValue is undefined reset as per inheritance. 
			if( this.sDivLines [i].value == 0 ){
				if(this.params.showSZeroPlaneValue == undefined){
					if(this.sDivLines [i].value ==  this.config.SYMin || this.sDivLines [i].value ==  this.config.SYMax ){
						this.params.showSZeroPlaneValue = this.params.showSecondaryLimits;
					}else{
						this.params.showSZeroPlaneValue = this.params.showDivLineSecondaryValue;
					}
				}
				this.sDivLines [i].showValue = showSDivValue = this.params.showSZeroPlaneValue;
			}
			...
			if (showSDivValue)
			{
				...
				//Create the text
				divLineValueObj = createText (false, this.sDivLines [i].displayValue, this.cMC, depth, xPos, yPos, 0, divLineFontObj, false, 0, 0);
				//Apply animation
				if (this.params.animation)
				{
	...
	*/
	private function drawZeroPlane (i) : Void
	{
		if (this.params.showZeroPlane)
		{
			...
			//Set alignment
			zeroPlaneFontObj.align = (this.params.primaryAxisOnLeft==true)?"right":"left";
			...
			//Calculate the x-position
			var xPos:Number = (this.params.primaryAxisOnLeft==true)?(this.elements.canvasBg.x - this.params.yAxisValuesPadding):(this.elements.canvas.toX + this.params.yAxisValuesPadding);
			//Render the line
			...
			{
				//Create the text
				var zeroPlaneValueObj : Object = createText (false, this.divLines [i].displayValue, this.cMC, zpVDepth, xPos, yPos, 0, zeroPlaneFontObj, false, 0, 0);
			}
			//Apply animation and filter effects to div line (y-axis) values
			if (this.divLines [i].showValue)
			{

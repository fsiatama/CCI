...
class com.fusioncharts.core.charts.MSColumn3DLineDYChart extends DualYAxis3DChart 
{
			...
			if (arrDocElement [i].nodeName.toUpperCase () == "GRAPH" || arrDocElement [i].nodeName.toUpperCase () == "CHART")
			{
				...
				for (j = 0; j < arrLevel1Nodes.length; j ++)
				{
					...
					} else if (arrLevel1Nodes [j].nodeName.toUpperCase () == "DATASET")
					{
						...
						//store the status of anchors in the dataset
						...
						if(this.dataset [this.numDS].includeInLegend){
							...
							//If secondary y axis
								...
								//If any anchor or line is visible legend item should be shown.
	...
	*/
	private function parseAttributes (graphElement : XMLNode) : Void 
	{
		...
		this.params.sYAxisValueDecimals = atts ["syaxisvaluedecimals"];
	}
	/**
	...
	*/
	private function calculatePoints (isRedraw:Boolean)
	{
					...
					switch(this.dataset [j].parentYAxis){
						...
						case 'S':
							...
							if(this.dataset[j].drawAnchors){
								...
								//if all anchors are invisible the legend icon appears without anchors.
		...
		labelMetrics.canvasPadding = this.params.canvasPadding + ((blockWidth) / 2) + this.config.blockSpace;
		...
		if(this.params.XTLabelManagement){
			...
			var newBlockSpaceWidth:Number = ( plotSpaceRatio / (this.num - 1) * metricsObj.totalInterPlotWidth);
			
			//Check the minimum value between newColumnWidth & maxColWidth
			var actualColumnWidth = Math.min(newColumnWidth, this.params.maxColWidth);
			var diffColWidth:Number = (newColumnWidth - actualColumnWidth);
			//When difference is more than 0 need to modify the blockwidth, newColumnwidth & newBlockSpaceWidth according to it
			if(diffColWidth > 0 && colWidth == this.params.maxColWidth ){
				newBlockWidth = newBlockWidth - (diffColWidth*this.numColDS);
				newColumnWidth = actualColumnWidth;
				newBlockSpaceWidth = newBlockSpaceWidth+(diffColWidth*this.numColDS*this.num)/(this.num + 1);
			}
		...
		for (i = 1; i <= this.num; i ++)
		{
			...
			if(this.params.XTLabelManagement){
				this.categories [i].x = this.elements.canvasBg.x + (amountOfDiff / 2) + (newBlockSpaceWidth * (i)) + (newBlockWidth * (i - 0.5)) - this.config.shifts.xShift - this.params.canvasBgDepth/2;
			}else{
			...
			for (j = 1; j <= this.numDS; j ++)
			{
				if (this.dataset [j].parentYAxis == "P")
				{
					...
					if(this.params.XTLabelManagement){
						this.dataset [j].data [i].x = this.elements.canvasBg.x + (amountOfDiff / 2) + (newBlockSpaceWidth * (i )) + (newBlockWidth * (i - 1)) + (newColumnWidth * (colCounter - 0.5)) - this.config.shifts.xShift - this.params.canvasBgDepth/2;
					}else{
					...
					//Width
					//Due to column shadow, especially for 3D looks the gap between two columns in same block is needed. The gap is minimized on very small plotspacepercent.
					shadowW = (this.config.shifts.wShadow*0.75)*((this.params.plotSpacePercent < 5)? (this.params.plotSpacePercent/10): 1);
					shadowW = (shadowW > (colWidth*0.5))? (colWidth*0.5): shadowW;
					this.dataset [j].data [i].w = colWidth - shadowW;
		...
		for (i = 1; i <= this.num; i ++)
		{
			for (j = 1; j <= this.numDS; j ++)
			{
				...
				if (this.dataset [j].parentYAxis == "P" && this.dataset [j].data [i].isDefined)
				{
					...
					colIns = new Column3D (colMC, this.dataset [j].data [i].x, this.dataset [j].data [i].y, this.dataset [j].data [i].w, this.dataset [j].data [i].h, this.config.shifts.xShift, this.config.shifts.yShift, this.dataset [j].data [i].color, this.params.showPlotBorder, this.params.plotBorderColor, this.params.plotBorderAlpha);
					//Draw the column

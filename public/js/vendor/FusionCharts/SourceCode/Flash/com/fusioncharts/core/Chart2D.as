...
class com.fusioncharts.core.Chart2D extends SingleYAxis2DVerticalChart {
	...
	//To map items of dataset to lgnd 
	private var arrdataLgndMap:Array;
	//Redrawing is on
	private var redrawing:Boolean;
	//To track legend item properties
	private var arrLegendDetail:Array;
	...
	*/
	private function parseAttributes(graphElement:XMLNode):Void {
		...
		//-----------------------------------------------//     
		//---- CONFIGURE AXIS TICKS FOR LABELS  ---------//
		// For x-axis tick
		this.params.xAxisTickColor = formatColor(getFV(atts["xaxistickcolor"], "dddddd"));
		this.params.xAxisTickAlpha = getFN(atts["xaxistickalpha"], 100);
		this.params.xAxisTickThickness = getFN(atts["xaxistickthickness"], 1);
	...
	*/
	private function calculatePoints():Void {
		var isRedraw:Boolean = this.redrawing;
		...
		for (i = 0; i < this.divLines.length; i++) {
			...
			if ((i == 0) || (i == this.divLines.length - 1)) {
				...
				if(this.params.showZeroPlaneValue){
					//Get the width of the text
					divLineObj = createText(true, this.divLines[i].displayValue, this.tfTestMC, 1, testTFX, testTFY, 0, divStyle, false, 0, 0);
					//Accomodate
					yAxisValMaxWidth = (divLineObj.width > yAxisValMaxWidth) ? (divLineObj.width) : (yAxisValMaxWidth);
		...
		if (this.params.showLegend) {
			...
			if(!isRedraw){
				...
				//Array to map items from dataset to legend
				this.arrdataLgndMap = [];
				this.arrLegendDetail = [];
				//Getting the height of legend item textfield in single line.
				var iconHeight:Number = lgnd.getIconHeight()*this.params.legendIconScale;
				var lgndItemId:Number;
				//Feed data set series Name for legend
				for (var i = arrdataLgnd.length -1 ; i >= 0; i--) {
					var dataObj:Object = arrdataLgnd[i];
					//Validation be data be set in lgend
					if (dataObj['includeInLegend'] && dataObj['seriesName'] != "") {
						//The cosmetics to draw icons
						var objIcon:Object = {fillColor: dataObj['color'], lineColor: dataObj['color'], intraIconPaddingPercent: 0.3};
						//Get the icons
						var objIcons:Object = LegendIconGenerator.getIcons(LegendIconGenerator[(dataObj['type'])], iconHeight, false, objIcon);
						//Setting the icon bitmaps for the 2 states
						var bmpd1:BitmapData = objIcons.active;
						var bmpd2:BitmapData = objIcons.inactive;
						
						//If COLUMNs and clustered
						if(this.params.clustered && dataObj['type'] == "COLUMN"){
							lgndItemId = arrdataLgnd.length - 1 - i;
						//If not COLUMNs or its COLUMN but not clustered.
						}else {
							lgndItemId = i;
						}
						//Adding data items to legend
						lgnd.addItem (dataObj['seriesName'], lgndItemId, !dataObj['showValues'], bmpd1, bmpd2);
						//Mapping the data item between dataset and legend
						arrdataLgndMap[lgndItemId] = dataObj.index;
						this.arrLegendDetail [i] =  new Object();
						this.arrLegendDetail [i].datasetName = dataObj['seriesName'];
						this.arrLegendDetail [i].datasetIndex = dataObj.index;
					}
				}
			//If its resizing
			}else{
				//Reset previous legend 'items' array
	...
	private function sortForLgnd():Array {
		...
		if(this.params.clustered){
			//Create arrays to hold areas, columns and lines in seperately
			var arrAreas:Array = [];
			var arrLines:Array = [];
		...
		for (var i = 1; i < this.dataset.length; ++i) {
			...
			//If be clustered
				...
				//Seperate out data w.r.t. its type
				switch(arrSeriesData['type']){
					case "AREA":
						arrAreas.push(arrSeriesData);
					break;
					
					case "LINE":
						arrLines.push(arrSeriesData);
					break;
					
					case "COLUMN":
						//Add an extra id for sorting of columns
						arrSeriesData['clusterId'] = i;
						arrColumns.push(arrSeriesData);
					break;
				}
			}
		}
		...
		if(this.params.clustered){
			//Sort columns for proper clustering order
			arrColumns.sortOn('clusterId', Array.DESCENDING | Array.NUMERIC);
			//Fuse all seperate data together
			arrData = arrColumns.concat(arrAreas, arrLines);
		}

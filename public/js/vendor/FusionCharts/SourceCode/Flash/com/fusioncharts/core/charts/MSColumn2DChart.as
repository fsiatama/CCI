	...
	private function calculatePoints (isRedraw:Boolean)
	{
		...
		for (i = 1; i <= this.num; i ++)
		{
			...
			for (j = 1; j <= this.numDS; j ++)
			{
				//X-Position
				if(this.params.XTLabelManagement){
					columnWidth = newColumnWidth / this.numDS;
					this.dataset [j].data [i].x = this.elements.canvas.x + (amountOfDiff/2) + (newSpaceWidth * i) + columnWidth * (j - 0.5) + (columnWidth * this.numDS * (i - 1));
				}else{

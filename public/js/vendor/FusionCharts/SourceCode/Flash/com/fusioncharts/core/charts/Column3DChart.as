...
class com.fusioncharts.core.charts.Column3DChart extends SingleYAxis3DChart 
{
			...
			if (arrDocElement [i].nodeName.toUpperCase () == "GRAPH" || arrDocElement [i].nodeName.toUpperCase () == "CHART")
			{
				...
				for (j = 0; j < arrLevel1Nodes.length; j ++)
				{
					...
					if (arrLevel1Nodes [j].nodeName.toUpperCase () == "SET")
					{
						...
						var setColor : String = formatColor (getFV (atts ["color"] , this.defColors.getColor ()));
	...
	*/
	private function calculatePoints ()
	{
		...
		if(this.data [1].showLabel){
			if(this.params.labelDisplay == "ROTATE"){
				labelObj = getTextMetrics(this.data [1].label, labelStyleObj, maxAllowableLabelHeight);
				labelMetrics.firstLabelWidth = labelObj.textFieldHeight;				
			}else{
				labelObj = getTextMetrics(this.data [1].label, labelStyleObj);
				labelMetrics.firstLabelWidth = labelObj.textFieldWidth;
			}
		}else{
			labelMetrics.firstLabelWidth = 0;
		}
		if(this.data [this.num].showLabel){
			if(this.params.labelDisplay == "ROTATE"){
				labelObj = getTextMetrics(this.data [this.num].label, labelStyleObj, maxAllowableLabelHeight);
				labelMetrics.lastLabelWidth = labelObj.textFieldHeight;				
			}else{
				labelObj = getTextMetrics(this.data [this.num].label, labelStyleObj);
		...
		if (this.params.labelDisplay == "ROTATE")
		{
			...
			for (i = 1; i <= this.num; i ++)
			{
				...
				if (this.data [i].showLabel)
				{
					...
					labelObj = createText (true, this.data [i].label, this.tfTestMC, 1, testTFX, testTFY, this.config.labelAngle, labelStyleObj, true, maxLabelWidth, maxLabelHeight);
					//Store the larger
		...
		{
			//Create element for caption - to store width & height
			this.elements.caption = returnDataAsElement (0, 0, canvasWidth, this.elements.caption.h);
		}
		//For sub-caption
		if (this.params.subCaption != "")
		{
			//Create element for sub caption - to store height
			this.elements.subCaption = returnDataAsElement (0, 0, canvasWidth, this.elements.subCaption.h);
		}
		...
		//No columspace should less than zero
		colSpace = ( colSpace < 0 )? 0 : colSpace;
		...
		{
			// All Negative number present - so set basePlanePos as yMax value
		...
		for (i = 1; i <= this.num; i ++)
		{
			...
			if(this.params.XTLabelManagement){
				this.data [i].x = this.elements.canvasBg.x + (amountOfDiff/2) + (newSpaceWidth * i ) + ((i - 0.5) * newColumnWidth) - this.config.shifts.xShift - this.params.canvasBgDepth/2;
			}else{
	...
	**/
	 ...
	 */
	 private function drawExtremeLabels()
	 {
		...
		labelRenderer.renderXAxisLabels(this.cMC, depth, this.num, this.data, labelStyleObj, this.config, this.params.labelDisplay,
		...
		for (i = 1; i <= this.num; i ++)
		{
			...
			if (this.data [i].showLabel)
			{
				if (this.params.labelDisplay == "ROTATE")
				{
					...
					//Create text box and get height
					labelObj = createText (false, this.data [i].label, this.cMC, depth, this.data [i].x, this.elements.canvas.toY + this.params.labelPadding, this.config.labelAngle, labelStyleObj, true, this.config.wrapLabelWidth, this.config.wrapLabelHeight);
				} else if (this.params.labelDisplay == "WRAP")
				{
					//Case 2 (WRAP)
					//Set alignment
	...
	private function drawColumns ()
	{
		...
		for (i = 1; i <= this.num; i ++)
		{
			...
			if (this.data [i].isDefined)
			{
				...
				colIns = new Column3D (colMC, this.data [i].x, this.data [i].y, this.data [i].w, this.data [i].h, this.config.shifts.xShift, this.config.shifts.yShift, this.data [i].color, this.params.showPlotBorder, this.params.plotBorderColor, this.params.plotBorderAlpha);
				//Draw the column

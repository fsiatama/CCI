			...
			case 'AREA' :
				...
				for (var j = 0; j < num1; j++) {
					...
					if (checkBlockInit) {
						...
						//Setting a presentative last previous (non-zero) value with proper sign
						var prevValue:Number = (arrBlockSigns[arrBlockSigns.length - 1])? -1 : 1;
						//Check to see if zero plane intecepts
						if (dataValue * prevValue < 0) {
							// set shifting flag
							arrBlockSigns.push((dataValue > 0) ? false : true);
						}
					}

...
class com.fusioncharts.core.charts.Area2DChart extends SingleYAxis2DVerticalChart 
{
			...
			if (arrDocElement [i].nodeName.toUpperCase () == "GRAPH" || arrDocElement [i].nodeName.toUpperCase () == "CHART")
			{
				...
				for (j = 0; j < arrLevel1Nodes.length; j ++)
				{
					...
					if (arrLevel1Nodes [j].nodeName.toUpperCase () == "SET")
					{
						...
						var setColor : String = formatColor (getFV (atts ["color"] , this.params.plotFillColor));
	...
	*/
	private function parseAttributes (graphElement : XMLNode) : Void 
	{
		...
		this.params.plotFillColor = formatColor (getFV (atts ["plotfillcolor"] , atts ["areabgcolor"] , ((this.params.paletteColors!="")?(paletteColor):(this.defColors.get2DPlotFillColor (this.params.palette)))));
		...
		this.params.plotBorderColor = formatColor (getFV (atts ["plotbordercolor"] , atts ["areabordercolor"] , ((this.params.paletteColors!="")?(paletteColor):(this.defColors.get2DPlotBorderColor (this.params.palette)))));
	...
	*/
	private function calcVLinesPos ()
	{
		...
		for (var i:Number = 1; i <= numVLines; i ++)
		{
			...
			if (this.data [i].showLabel)
			{
				...
				if (this.num>1 && count==this.config.finalVisibleCount){					
					...
					if (this.params.labelDisplay == "WRAP"){
						...
						if ((this.data [i].x+labelHeight)>(this.width-this.params.chartRightMargin)){												
							//Allot max available space available between this and previous label.

		...
		for (i=1; i<=this.numDS; i++) {
			for (j=1; j<=this.dataset[i].numSDS; j++) {
				for (k=1; k<=this.num; k++) {
					if (this.dataset[i].dataset[j].data[k] == undefined) {
						this.dataset[i].dataset[j].data[k] = this.returnDataAsObject(NaN);
					}
				}
			}
		}
	...
	private function preparePercentData():Void {
		...
		if (this.params.stack100Percent) {
			...
			for (i=1; i<=this.numDS; i++) {
				for (j=1; j<=this.dataset[i].numSDS; j++) {
					for (k=1; k<=this.num; k++) {
						if (this.dataset[i].dataset[j].data[k].isDefined) {
							//Calculate the % value of the data
							percentValue = (this.dataset[i].dataset[j].data[k].value/ ((this.dataset[i].sums[k].sum == 0)? 1: this.dataset[i].sums[k].sum))*100;
							...
							if (this.dataset[i].dataset[j].data[k].exDispVal == "") {
								...
								if (this.params.showPercentValues) {
									//Store % value
									this.dataset[i].dataset[j].data[k].exDispVal = formatNumber(percentValue, this.params.formatNumber, this.params.decimals, this.params.forceDecimals, false, this.params.defaultNumberScale, this.config.nsv, this.config.nsu, "", "%");
								} else {
									//Store formatted value
									this.dataset[i].dataset[j].data[k].exDispVal = formatNumber(this.dataset[i].dataset[j].data[k].value, this.params.formatNumber, this.params.decimals, this.params.forceDecimals, this.params.formatNumberScale, this.params.defaultNumberScale, this.config.nsv, this.config.nsu, this.params.numberPrefix, this.params.numberSuffix, this.params.scaleRecursively, this.params.scaleSeparator, this.params.maxScaleRecursion);
								}
							}
							//Also store tool-tip value 
							if (this.params.showPercentInToolTip) {
								//Store % value
								this.dataset[i].dataset[j].data[k].toolTipDispVal = formatNumber(percentValue, this.params.formatNumber, this.params.decimals, this.params.forceDecimals, false, this.params.defaultNumberScale, this.config.nsv, this.config.nsu, "", "%");
							} else {
								//Store formatted value
								this.dataset[i].dataset[j].data[k].toolTipDispVal = formatNumber(this.dataset[i].dataset[j].data[k].value, this.params.formatNumber, this.params.decimals, this.params.forceDecimals, this.params.formatNumberScale, this.params.defaultNumberScale, this.config.nsv, this.config.nsu, this.params.numberPrefix, this.params.numberSuffix, this.params.scaleRecursively, this.params.scaleSeparator, this.params.maxScaleRecursion);
							}
							//Store the % value, for calculations
							this.dataset[i].dataset[j].data[k].value = percentValue;
							//Store a copy of original value
							this.dataset[i].dataset[j].data[k].origValue = this.dataset[i].dataset[j].data[k].value;
							//Round to 1 decimal on lower side
							//To avoid the decimal summation of >100.00000x%
							this.dataset[i].dataset[j].data[k].value = int(this.dataset[i].dataset[j].data[k].value*10)/10;
						}
					}
				}
			}
			...
			for (i=1; i<=this.num; i++) {
				for (j=1; j<=this.numDS; j++) {
					this.dataset[j].sums[i].displayValue = formatNumber(this.dataset[j].sums[i].sum, this.params.formatNumber, this.params.decimals, this.params.forceDecimals, this.params.formatNumberScale, this.params.defaultNumberScale, this.config.nsv, this.config.nsu, this.params.numberPrefix, this.params.numberSuffix, this.params.scaleRecursively, this.params.scaleSeparator, this.params.maxScaleRecursion);
				}
			}
	...
	*/
	private function calculateAxisLimits():Void {
		...
		if (!isNaN(this.params.yAxisMinValue)) {
			...
			for (i=1; i<=this.numDS; i++) {
				//Check whether the first dataset's minimum value is lower than  yAxisMinValue or not
				for (j=1; j<=this.dataset[i].numSDS; j++) {
					//Mark yAxisMinValue as undefined - Only if the below cases are true.
					//Case 1 - In each stack if first dataset's any value is less than the yAxisMinValue.
					...
					minValueToCompare = (j == 1 || this.params.yAxisMinValue < 0)? this.params.yAxisMinValue : 0;
					for (k=1; k<=this.num; k++) {
	...
	*/
	private function calculatePoints(isRedraw:Boolean) {
			...
			for (j=1; j<=this.dataset[i].numSDS; j++) {
				for (k=1; k<=this.num; k++) {
					if(this.params.showPercentValues || this.params.showPercentInToolTip){
						//Calculate the % value of the data
						var percentValue = (this.dataset[i].dataset[j].data[k].value / ((this.dataset[i].sums[k].sum == 0)? 1: this.dataset[i].sums[k].sum)) * 100;
					}
					//Format and store
					if(this.params.showPercentValues){
						this.dataset[i].dataset[j].data[k].displayValue = formatNumber(percentValue, this.params.formatNumber, this.params.decimals, this.params.forceDecimals, this.params.formatNumberScale, this.params.defaultNumberScale, this.config.nsv, this.config.nsu, "", "%");
					}else{
					...
					if (this.params.showSum && (j == 1) && this.dataset[i].sums[k].displayValue == undefined) {
					...
					if (this.dataset[i].dataset[j].data[k].toolText == undefined || this.dataset[i].dataset[j].data[k].toolText == "") {
						...
						//toolText = toolText+((this.params.stack100Percent == true) ? (this.dataset[i].dataset[j].data[k].toolTipDispVal) : (this.dataset[i].dataset[j].data[k].displayValue));
						if(this.params.stack100Percent){
							toolText = toolText + this.dataset[i].dataset[j].data[k].toolTipDispVal
						}else{
							if(this.params.showPercentInToolTip){
								...
								//actual value
								toolText = toolText + formatNumber(this.dataset[i].dataset[j].data[k].value, this.params.formatNumber, this.params.decimals, this.params.forceDecimals, this.params.formatNumberScale, this.params.defaultNumberScale, this.config.nsv, this.config.nsu, this.params.numberPrefix, this.params.numberSuffix, this.params.scaleRecursively, this.params.scaleSeparator, this.params.maxScaleRecursion);
							}
						}
		...
		if (columnWidth>50 && this.defaultPlotSpacePercent) {
			...
			this.params.plotSpacePercent = (plotSpace * 100) / this.elements.canvas.w;
		}

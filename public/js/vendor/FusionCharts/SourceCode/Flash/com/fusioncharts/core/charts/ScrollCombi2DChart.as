					...
					} else if (arrLevel1Nodes [j].nodeName.toUpperCase () == "DATASET")
					{
						...
						for (k = 0; k < arrLevel2Nodes.length; k ++)
						{
							if (arrLevel2Nodes [k].nodeName.toUpperCase () == "SET")
							{
								...
								var isColumnDS:Boolean = (this.dataset [this.numDS].renderAs == "COLUMN")? true : false;
	...
	*/
	private function calculatePoints (isRedraw:Boolean)
	{
		...
		if(this.scrollRequired){
			...
			//this.config.labelAreaHeight = Math.min(this.config.maxLabelHeight, this.config.maxAllowableLabelHeight);
		...
		if (this.numColDS > 0)
		{
			...
			if(this.scrollRequired){
				...
				//Based on canvas height that has been re-calculated, re-adjust yaxisLabelStep
				this.adjustYAxisLabelStep(yAxisValMaxHeight, canvasHeight);
				//Recalculating  caption and subcaption width so that it takes maximum canvas width 
				//For caption
				if (this.params.caption != "")
				...
				//Recreate an element to represent the canvas now.
				this.elements.canvas = returnDataAsElement (canvasStartX, canvasStartY, canvasWidth, canvasHeight);

...
class com.fusioncharts.core.charts.StackedBar2DChart extends SingleYAxis2DHorizontalChart 
{
	...
	*/
	private function parseAttributes (graphElement : XMLNode) : Void 
	{
		...
		//Sum value padding - from right canvas
	...
	*/
	private function calculatePoints (isRedraw:Boolean)
	{
		...
		this.adjustYAxisLabelStep(yAxisValMaxWidth, canvasWidth);
		...
		} else if (this.config.yMax < 0 && this.config.yMin < 0)
		{
			...
			this.config.basePlanePos = this.getAxisPosition (this.config.yMax, this.config.yMax, this.config.yMin, this.elements.canvas.x, this.elements.canvas.toX, false, 0);
		} else 
		{
		...
		//been defined
		if (barHeight > 50 && this.defaultPlotSpacePercent)
		{
			barHeight = 50;
		...
		for (i = 1; i <= this.num; i ++)
		{
			...
			for (j = 1; j <= this.numDS; j ++)
			{
				if (this.dataset [j].data [i].isDefined)
				{
					...
					dataEndX = this.getAxisPosition (dataValue, this.config.yMax, this.config.yMin, this.elements.canvas.x, this.elements.canvas.toX, false, 0);
					//Get the width.
	...
	*/
	private function drawHeaders ()
	{
		...
		if (this.params.yAxisName != "")
		{
			...
			var yAxisNameObj : Object = createText (false, this.params.yAxisName, this.cMC, this.dm.getDepth ("YAXISNAME") , this.width/2 , this.height - this.params.chartBottomMargin - this.config.legendHeight, 0, yAxisNameStyleObj, true, this.width - (this.params.chartLeftMargin+this.params.chartRightMargin), this.elements.yAxisName.h);
			//Apply animation
			if (this.params.animation)
			{
				...
				if (this.dataset [i].data [j].isDefined && this.dataset [i].data [j].showValue)
				{
					...
					if(this.dataset [i].data [j].displayValueDefined){
						...
						if(valueObj.width > Math.abs(this.dataset [i].data [j].w)){
							...
							valueObj = createText (false, this.dataset [i].data [j].displayValue, this.cMC, depth, xPos, this.dataset [i].data [j].y, angle, valueStyleObj, true, maxTextWidth, valueLabelHeight);
						}
					}else{
						valueObj = createText (false, this.dataset [i].data [j].displayValue, this.cMC, depth, xPos, this.dataset [i].data [j].y, angle, valueStyleObj, false, 0, 0);
					}

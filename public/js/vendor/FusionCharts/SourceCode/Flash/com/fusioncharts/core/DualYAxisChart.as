...
class com.fusioncharts.core.DualYAxisChart extends Chart 
{
		...
		if ( ! (this.params.sNumberScaleValue.length == 0 || this.params.sNumberScaleUnit.length == 0 || this.params.sFormatNumberScale == false))
		{
			...
			for (i = 0; i < this.config.snsv.length; i ++)
			{
				...
				if (isNaN (this.config.snsv [i]))
				{
					...
					this.params.sScaleRecursively = false;
				}
			}
			//Parse the number scale unit
			...
			//Push the default scales at start - Value as 1 (universal divisor)			
			if(this.params.sScaleRecursively){
				this.config.snsv.push(1);
				this.config.snsu.unshift(this.params.sDefaultNumberScale);
			}
		}
	...
	*							be less than 0.
	*/
	private function getPAxisLimits (maxValue : Number, minValue : Number, setMinAsZero : Boolean, stopMaxAtZero : Boolean ) : Void 
	{
		//If stopMaxAtZero not defined set it as true
		stopMaxAtZero = (stopMaxAtZero == undefined)? true: stopMaxAtZero;
		...
		//and lower limit.
		if (isNaN(this.params.PYAxisMaxValue))
		{
		...
		if (this.config.PYMaxGiven == false || (this.config.PYMaxGiven == true && Number (this.params.PYAxisMaxValue) < maxValue && isMaxValid))
		{
		...
		if (this.config.PYMinGiven == false || (this.config.PYMinGiven == true && Number (this.params.PYAxisMinValue) > minValue && isMinValid))
		{
		...
		//If primary ymin is greater than primary ymax then reset it
		if(this.config.PYMin >= this.config.PYMax){
			this.config.PYMax = this.config.PYMax + this.config.PYMin;
		}
		//Store axis range
	...
	*							be less than 0.
	*/
	private function getSAxisLimits (maxValue : Number, minValue : Number, setMinAsZero : Boolean, stopMaxAtZero : Boolean) : Void 
	{
		//Flag variable to keep track on parameters values
		var isMinValid:Boolean = true;
		var isMaxValid:Boolean = true;
		...
		//Now, we need to make a check as to whether the user has provided an upper limit
		//and lower limit.
		if (isNaN(this.params.SYAxisMaxValue))
		{
		...
		if (isNaN(this.params.SYAxisMinValue))
		{
		...
		//If he has provided it and it is valid, we leave it as the upper limit
		//Else, we enforced the value calculate by us as the upper limit.
		if (this.config.SYMaxGiven == false || (this.config.SYMaxGiven == true && Number (this.params.SYAxisMaxValue) < maxValue && isMaxValid))
		{
		...
		//Now, we do the same for y-axis lower limit
		if (this.config.SYMinGiven == false || (this.config.SYMinGiven == true && Number (this.params.SYAxisMinValue) > minValue  && isMinValid))
		{
		...
		//If secondary ymin is greater than secondary ymax then reset it
		if(this.config.SYMin >= this.config.SYMax){
			this.config.SYMax = this.config.SYMax + this.config.SYMin;
		}
		//Store axis range
	...
	/**
	 * Syncs the primary and secondary axis so that they've same limits.
	*/
	private function syncAxisLimits():Void{
		//Find the min and max of both axis
		var min:Number, max:Number;
		//Find min
		if (isNaN(this.config.PYMin)){
			min = this.config.SYMin;
		}else if (isNaN(this.config.SYMin)){
			min = this.config.PYMin;
		}else{
			min = Math.min(this.config.PYMin, this.config.SYMin);
		}
		//Find max
		if (isNaN(this.config.PYMax)){
			max = this.config.SYMax;
		}else if (isNaN(this.config.SYMax)){
			max = this.config.PYMax;
		}else{
			max = Math.max(this.config.PYMax, this.config.SYMax);
		}
		//Set both the axis to same
		this.config.PYMin = this.config.SYMin = min;
		this.config.PYMax = this.config.SYMax = max;
		//Re-calculate intervals
		//And for 102, it will be 2 = 10^2 + 2
		var maxPowerOfTen : Number = Math.floor (Math.log (Math.abs (max)) / Math.LN10);
		//Get the minimum power of 10 that is applicable to maxvalue
		var minPowerOfTen : Number = Math.floor (Math.log (Math.abs (min)) / Math.LN10);
		//Find which powerOfTen (the max power or the min power) is bigger
		//It is this which will be multiplied to get the y-interval
		var powerOfTen : Number = Math.max (minPowerOfTen, maxPowerOfTen);
		var y_interval : Number = Math.pow (10, powerOfTen);
		//For accomodating smaller range values (so that scale doesn't represent too large an interval
		if (Math.abs (max) / y_interval < 2 && Math.abs (min) / y_interval < 2)
		{
			powerOfTen --;
			y_interval = Math.pow (10, powerOfTen);
		}
		//If the y_interval of min and max is way more than that of range.
		//We need to reset the y-interval as per range
		var rangePowerOfTen : Number = Math.floor (Math.log (max - min) / Math.LN10);
		var rangeInterval : Number = Math.pow (10, rangePowerOfTen);
		//Now, if rangeInterval is 10 times less than y_interval, we need to re-set
		//the limits, as the range is too less to adjust the axis for max,min.
		//We do this only if range is greater than 0 (in case of 1 data on chart).
		if (((max- min) > 0) && ((y_interval / rangeInterval) >= 10))
		{
			y_interval = rangeInterval;
			powerOfTen = rangePowerOfTen;
		}
		//Store interval and new range
		this.config.PRange = Math.abs (this.config.PYMax - this.config.PYMin);
		this.config.SRange = Math.abs (this.config.SYMax - this.config.SYMin);
		//Store interval
		this.config.PInterval = y_interval;
		this.config.SInterval = y_interval;
		//Set manual over-ride flags
		this.config.PYMinGiven = true;
		this.config.PYMaxGiven = true;
		this.config.SYMinGiven = true;
		this.config.SYMaxGiven = true;
	...
	*/
	private function returnDataAsPDivLine (value : Number) : Object 
	{
		...
		//Display value
		divLineObject.displayValue = this.formatNumber (value, this.params.formatNumber, this.params.yAxisValueDecimals, this.params.forceYAxisValueDecimals, this.params.formatNumberScale, this.params.defaultNumberScale, this.config.nsv, this.config.nsu, this.params.numberPrefix, this.params.numberSuffix, this.params.scaleRecursively, this.params.scaleSeparator, this.params.maxScaleRecursion);
		//Flag to show values
	...
	*/
	private function returnDataAsSDivLine (value : Number) : Object 
	{
		...
		divLineObject.value = value;
		//Display value
		divLineObject.displayValue = this.formatNumber (value, this.params.sFormatNumber, this.params.sYAxisValueDecimals, this.params.forceSYAxisValueDecimals, this.params.sFormatNumberScale, this.params.sDefaultNumberScale, this.config.snsv, this.config.snsu, this.params.sNumberPrefix, this.params.sNumberSuffix,  this.params.sScaleRecursively, this.params.sScaleSeparator, this.params.sMaxScaleRecursion);
		//Flag to show values
		divLineObject.showValue = true;
		return divLineObject;
	}
	...
	*	@param	lineColorDefined	Line color defined explicitly
	*	@return					An object encapsulating these values.
	*/
	private function returnDataAsVLineObj (index : Number, label:String, showLabelBorder:Boolean, labelHAlign:String, labelVAlign:String, labelPosition:Number, linePosition:Number, color : String, thickness : Number, alpha : Number, isDashed : Boolean, dashLen : Number, dashGap : Number, lineColorDefined : Boolean) : Object 
	{
		//Create an object that will be returned.
		var rtnObj : Object = new Object ();
		//Store parameters as object properties
		...
		rtnObj.dashLen = dashLen;
		rtnObj.dashGap = dashGap;
		rtnObj.lineColorDefined = lineColorDefined;
	...
	*/
	private function parseVLineNode (vLineNode : XMLNode, index : Number)
	{
		...
		var color : String, thickness : Number, alpha : Number, lineColorDefined:Boolean;
		...
		lineColorDefined = (formatColor(lineAttr ["color"]) == color);
		thickness = getFN (lineAttr ["thickness"] , 1);
		alpha = getFN (lineAttr ["alpha"] , 80);
		isDashed = toBoolean (Number (getFV (lineAttr ["dashed"] , 0)));
		//As Flash draws line with rounded edges we need to set lineGap and lineLength
		//based on its thickness.
		var defaultDashLen : Number = 5 * thickness;
		var defaultDashGap : Number = 2 * thickness;
		dashLen = getFN (lineAttr ["dashlen"] , defaultDashLen);
		dashGap = getFN (lineAttr ["dashgap"] , defaultDashGap);
		...
		this.vLines [this.numVLines] = this.returnDataAsVLineObj (index, label, showLabelBorder, labelHAlign, labelVAlign, labelPosition, linePosition, color, thickness, alpha, isDashed, dashLen, dashGap, lineColorDefined);
	}
	/**
			...
			} else 
			{
				...
				if (this.trendLines [i].parentYAxis == "P")
				{
					...
					else
					{
						...
						this.trendLines [i].displayValue = getFV (this.trendLines [i].displayValue, this.formatNumber (this.trendLines [i].startValue, this.params.formatNumber, this.params.yAxisValueDecimals, this.params.forceYAxisValueDecimals, this.params.formatNumberScale, this.params.defaultNumberScale, this.config.nsv, this.config.nsu, this.params.numberPrefix, this.params.numberSuffix, this.params.scaleRecursively, this.params.scaleSeparator, this.params.maxScaleRecursion));
					}
				} 
				else
				{
					...
					{
						//We resolve displayValue conflict
						this.trendLines [i].displayValue = getFV (this.trendLines [i].displayValue, this.formatNumber (this.trendLines [i].endValue, this.params.sFormatNumber, this.params.sYAxisValueDecimals, this.params.forceSYAxisValueDecimals, this.params.sFormatNumberScale, this.params.sDefaultNumberScale, this.config.snsv, this.config.snsu, this.params.sNumberPrefix, this.params.sNumberSuffix, this.params.sScaleRecursively, this.params.sScaleSeparator, this.params.sMaxScaleRecursion));
					}
				}
			}
		}

...
class com.fusioncharts.core.charts.StackedColumn3DLineDYChart extends DualYAxis3DChart 
{
			...
			if (arrDocElement [i].nodeName.toUpperCase () == "GRAPH" || arrDocElement [i].nodeName.toUpperCase () == "CHART")
			{
				...
				for (j = 0; j < arrLevel1Nodes.length; j ++)
				{
					...
					} else if (arrLevel1Nodes [j].nodeName.toUpperCase () == "DATASET")
					{
						...
						var pAxis:String = getFV (dsAtts ["parentyaxis"] , "P");
						this.dataset [this.numDS].parentYAxis = pAxis.toUpperCase();
						//Parent y-axis can either be P or S - Defaults to P
						if (this.dataset [this.numDS].parentYAxis.toUpperCase() != "P" && this.dataset [this.numDS].parentYAxis.toUpperCase() != "S")
						{
						...
						this.dataset [this.numDS].color = formatColor (getFV (dsAtts ["color"] ,((this.dataset [this.numDS].parentYAxis == "S")? this.params.lineColor : undefined ), this.defColors.getColor ()));
						...
						this.dataset [this.numDS].anchorBorderColor = (dsAtts ["color"] == " " || dsAtts ["color"] == "#" )? "000000" : this.dataset [this.numDS].anchorBorderColor ;
						...
						for (k = 0; k < arrLevel2Nodes.length; k ++)
						{
							if (arrLevel2Nodes [k].nodeName.toUpperCase () == "SET")
							{
								...
								if(!anyPlotVisible){
									//If Column
									if(this.dataset [this.numDS].parentYAxis == "P"){
										if(!isNaN(setValue) && setAlpha > 0){
											anyPlotVisible = true;
										}
									//If Line
									}else{
										if(!isNaN(setValue) && setAlpha > 0 && (previousPlotAlpha > 0 || setShowValue)){
											anyPlotVisible = true;
										}
								...
								this.dataset [this.numDS].data [setCount] = this.returnDataAsObject (setValue, setColor, setExDispVal,  setAlpha, setToolText, setLink, setShowValue, setValuePosition, setDashed, setAnchorSides, setAnchorRadius, setAnchorBorderColor, setAnchorBorderThickness, setAnchorBgColor, setAnchorAlpha, setAnchorBgAlpha);
								...
								//As negativePresent & positivePresent used only for primary y axis there is no need to 
								//consider values that will be plotted on the secondary axis.
								if(this.dataset [this.numDS].parentYAxis == 'P'){
						...
						//store the anchor visible status in each set
	...
	*/
	private function parseAttributes (graphElement : XMLNode) : Void 
	{
		...
		//Whether axis positions would be swapped. By default, primary axis is on
		//left side.
		this.params.primaryAxisOnLeft = toBoolean (getFN (atts ["primaryaxisonleft"] , 1));
	...
	*/
	private function calculateAxisLimits () : Void 
	{
		...
		if (!isNaN(this.params.PYAxisMinValue)){
			...
			//Variabele for checking the first dataset of the primary y axis
			var firstPrimaryYAxisDataset:Boolean = false;
			//Store as number
			...
			for (i = 1; i <= this.numDS; i ++)
			{
				if (this.dataset [i].parentYAxis=="P" ){
					...
					//Case 1 - If first dataset's any value is less than the PYAxisMinValue.
					...
					minValueToCompare = (!firstPrimaryYAxisDataset || this.params.PYAxisMinValue < 0)? this.params.PYAxisMinValue: 0;
					firstPrimaryYAxisDataset = true;
					for (j = 1; j <= this.num; j ++)
					{
		...
		this.getSAxisLimits (this.getMaxSDataValue () , this.getMinSDataValue () , ! this.params.setAdaptiveSYMin , ! this.params.setAdaptiveSYMin);
		//Sync, if required
	...
	private function calculatePoints (isRedraw:Boolean)
	{
		...
		for (i = 1; i <= this.numDS; i ++)
		{
			for (j = 1; j <= this.num; j ++)
			{
				...
				{
					this.dataset [i].data [j].displayValue = formatNumber (this.dataset [i].data [j].value, this.params.sFormatNumber, this.params.sDecimals, this.params.sForceDecimals, this.params.sFormatNumberScale, this.params.sDefaultNumberScale, this.config.snsv, this.config.snsu, this.params.sNumberPrefix, this.params.sNumberSuffix, this.params.sScaleRecursively, this.params.sScaleSeparator, this.params.sMaxScaleRecursion);
				}
		...
		//Switch primary y-axis and secondary y-axis names (if primary axis is on right)
		if (this.params.primaryAxisOnLeft==false){
			var tmpName:String = this.params.PYAxisName;
			var tmpWidth:Number = this.params.PYAxisNameWidth;
			//Swap
			this.params.PYAxisName = this.params.SYAxisName;
			this.params.PYAxisNameWidth = this.params.SYAxisNameWidth;
			this.params.SYAxisName = tmpName;
			this.params.SYAxisNameWidth = tmpWidth;
		}
		...
		if (this.params.showLegend)
		{
			...
			for (i = 1; i <= this.numDS; i ++)
			{
				...
				if (this.dataset [j].includeInLegend && this.dataset [j].seriesName != "")
				{
					...
					switch(this.dataset [j].parentYAxis){
						case 'P':
					...
					var objIcons:Object = LegendIconGenerator.getIcons((this.dataset [j].parentYAxis=='P')?LegendIconGenerator.COLUMN:LegendIconGenerator.LINE, iconHeight, true, objIcon);
		...
		//---------------- No need to calculate shadow width  with columns as shadow is always outside columns #FCXT-282 -------- //
		...
		} else if (this.config.PYMax < 0 && this.config.PYMin < 0)
		{
			...
			this.config.basePlanePos = this.getAxisPosition (this.config.PYMax, this.config.PYMax, this.config.PYMin, this.elements.canvasBg.y, this.elements.canvasBg.toY, true, 0);
		} else 
		{
			...
			for (j = 1; j <= this.numDS; j ++)
			{
				if (this.dataset [j].parentYAxis == "P")
				{
					if (this.dataset [j].data [i].isDefined)
					{
						...
						dataEndY = this.getAxisPosition (dataValue, this.config.PYMax, this.config.PYMin, this.elements.canvasBg.y, this.elements.canvasBg.toY, true, 0);

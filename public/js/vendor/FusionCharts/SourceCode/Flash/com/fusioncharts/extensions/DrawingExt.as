...
class com.fusioncharts.extensions.DrawingExt {
	...
	*/
	...
	/**
	* drawSpoke method helps draw a spoke based on the parameters specified.
	*	Assumptions: Movie clip is already created and line/fill style has
	*				 already been set before this method is called to
	*				 draw a spoke.
	*	@param		mc		Movie clip in which we've to draw the spoke
	*	@param		x		Center X position of the spoke
	*	@param		sides	Number of sides required for spoke (min 3).
	*	@param		startAngle	Starting angle of the spoke in degrees. Default is 90.
	*/
	public static function drawSpoke(mc:MovieClip, x:Number, y:Number, sides:Number, radius:Number, startAngle:Number):Void {
		...
		if (sides>2) {
			...
			//Default for starting angle
			startAngle = (isNaN(startAngle)) ? 90 : startAngle;
			//Variables to store incremental angle
			var incAngle:Number;
			//Loop variable
			var i:Number;
			...
			for (i=0; i<sides; i++) {
				mc.moveTo(x, y);
				mc.lineTo(x+(Math.cos(startAngle + i*incAngle)*radius), y-(Math.sin(startAngle + i*incAngle)*radius));
			}
		}
	}

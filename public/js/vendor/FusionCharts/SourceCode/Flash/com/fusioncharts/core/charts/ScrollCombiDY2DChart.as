...
class com.fusioncharts.core.charts.ScrollCombiDY2DChart extends DualYAxis2DChart 
{
	...
	*/
	public function render (isRedraw:Boolean) : Void 
	{
		...
		this.parseXML ();
		//now to manage the chart type we keep it as column if any column dataset is active
		labelMetrics.chartType = (this.numColDS > 0) ? "column" : "line";
			...
			if (arrDocElement [i].nodeName.toUpperCase () == "GRAPH" || arrDocElement [i].nodeName.toUpperCase () == "CHART")
			{
				...
				for (j = 0; j < arrLevel1Nodes.length; j ++)
				{
					...
					} else if (arrLevel1Nodes [j].nodeName.toUpperCase () == "DATASET")
					{
						...
						this.dataset [this.numDS].dashed = toBoolean (getFN (dsAtts ["dashed"] , ((this.dataset [this.numDS].renderAs != "LINE")? this.params.plotBorderDashed : this.params.lineDashed)));
						...
						//store the anchor visible state in each dataset
	...
	*/
	...
	{
		//Calculate limits
		this.getPAxisLimits (this.getMaxDataValue ("P") , this.getMinDataValue ("P") , ! this.params.setAdaptiveYMin, ! this.params.setAdaptiveYMin);
		this.getSAxisLimits (this.getMaxDataValue ("S") , this.getMinDataValue ("S") , ! this.params.setAdaptiveSYMin , ! this.params.setAdaptiveSYMin);
		//Sync, if required
	...
	*/
	private function calculatePoints (isRedraw:Boolean)
	{
		...
		if (this.params.labelDisplay=="AUTO"){
			...
			var canvasTotalWidth:Number = canvasWidth*((this.num*((this.numColDS==0)?1:this.numColDS))/this.params.numVisiblePlot);
		...
		if(this.scrollRequired){
			...
			//------------------------------------EoF Scroll Mode Label Management-------------------------------------------//
		}else {
			//---------------------------- For Non Scroll Mode --------------------------------//
			...
			if (this.params.labelDisplay == "ROTATE")
			{
				...
				var maxLabelHeight : Number = (canvasWidth*((this.num*((this.numColDS==0)?1:this.numColDS))/this.params.numVisiblePlot)/ finalVisibleCount);
		...
		if (this.numColDS > 0)
		{
			...
			if(this.params.numVisiblePlot % this.numColDS == 0){
				numSpacesVisible = (this.params.numVisiblePlot/this.numColDS) + 1;
				unitSpaceWidth = visibleSpaceWidth/numSpacesVisible;
			} else {
				numSpacesVisible = Math.ceil(this.params.numVisiblePlot/this.numColDS);
				unitSpaceWidth = visibleSpaceWidth/numSpacesVisible;
			}
			...
			var interBlockSpace : Number = unitSpaceWidth;
			//Store in config.
			this.config.interBlockSpace = interBlockSpace;
			this.config.columnWidth = columnWidth;
			this.config.blockWidth = columnWidth * this.numColDS;
			...
			if(this.params.XTLabelManagement && !this.scrollRequired){
				...
				var newColumnWidth:Number = newBlockWidth / this.numColDS;
			...
			if(this.scrollRequired){
				...
				//Create an element to represent the canvas now.
				this.elements.canvas = returnDataAsElement (canvasStartX, canvasStartY, canvasWidth, canvasHeight);
				//Recalculate Base Plane position - Base plane is the y-plane from which columns start.
				//If there's a 0 value in between yMin,yMax, base plane represents 0 value.
				//Else, it's yMin
				if (this.config.PYMax >= 0 && this.config.PYMin < 0)
				{
			...
			for (i = 1; i <= this.num; i ++)
			{
				...
				if(this.params.XTLabelManagement && !this.scrollRequired){
					this.categories [i].x = (newSpaceWidth * i) + (newColumnWidth * this.numColDS * (i - 0.5));
				}else{
				...
				for (j = 1; j <= this.numDS; j ++)
				{
					...
					if (this.dataset [j].renderAs == "COLUMN")
					{
						...
						//X-Position
						if(this.params.XTLabelManagement && !this.scrollRequired){
							this.dataset [j].data [i].x = (newSpaceWidth * i) + newColumnWidth * (colCounter - 0.5) + (newColumnWidth * this.numColDS * (i - 1));
						}else{
		...
		else
		{
			...
			if (this.scrollRequired){
				var interPointWidth : Number = (this.elements.canvas.w - this.params.canvasPadding) / (this.params.numVisiblePlot - 1);
			}else{
	...
	private function getSimulationWidth(){
		...
		for (i = 1; i <= this.num; i ++)
		{
			...
			if(this.categories [i].showLabel){
				...
				{
					var maxLabelWidth : Number = this.config.maxAllowableLabelHeight;
					var maxLabelHeight : Number = (this.numColDS > 0)? (this.config.plotSpace + this.config.blockWidth*this.num) : ((2*this.params.canvasPadding)+ (this.config.interPointWidth*(this.num-1)));

...
class com.fusioncharts.core.charts.StackedBar3DChart extends SingleYAxis3DHorizontalChart
{
	...
	*/
	private function returnDataAsObject (value : Number, color : String, displayValue:String, alpha : Number, toolText : String, link : String, showValue : Number) : Object
	{
		...
		//No explicitly set display value defined
		dataObj.displayValueDefined = (displayValue == "")? false : true;
					...
					if (this.dataset [i].data [j].isDefined){
						...
						if (this.dataset[i].data[j].exDispVal==""){
							...
							if (this.params.showPercentValues){
								//Store % value
								this.dataset[i].data[j].exDispVal = formatNumber (percentValue, this.params.formatNumber, this.params.decimals, this.params.forceDecimals, false, this.params.defaultNumberScale, this.config.nsv, this.config.nsu, "", "%", this.params.scaleRecursively, this.params.scaleSeparator, this.params.maxScaleRecursion);
							}else{
						...
						if (this.params.showPercentInToolTip){
							//Store % value
							this.dataset[i].data[j].toolTipDispVal = formatNumber (percentValue, this.params.formatNumber, this.params.decimals, this.params.forceDecimals, false, this.params.defaultNumberScale, this.config.nsv, this.config.nsu, "", "%", this.params.scaleRecursively, this.params.scaleSeparator, this.params.maxScaleRecursion);
						}else{
							//Store formatted value
	...
	*/
	private function calculatePoints (isRedraw:Boolean)
	{
		...
		//Reserve space for showing sum of data (in case of 100% chart).
		if (this.params.showSum == true && this.params.stack100Percent==true)
		{
			var maxSumWidth:Number = 0;
			var sumObj:Object;
			var sumStyle:Object = this.styleM.getTextStyle (this.objects.DATAVALUES);
			//Iterate through all
			for (i=1; i<=this.num; i++){
				//Create text field to get width
				var sumObj : Object = createText (true, this.sums [i].displayValue, this.tfTestMC, 1, testTFX, testTFY, 0, sumStyle , false, 0, 0);
				maxSumWidth = Math.max(maxSumWidth, sumObj.width);
			}
			//Store the width
			canvasWidth = canvasWidth - maxSumWidth - this.params.sumPadding;
			delete sumObj;
		}
		...
		var dataEndX : Number, dataValue:Number;
		...
		for (i = 1; i <= this.num; i ++)
		{
			...
			for (j = 1; j <= this.numDS; j ++)
			{
				if (this.dataset [j].data [i].isDefined)
				{
					...
					//Width for each bar
					dataEndX = this.getAxisPosition (dataValue, this.config.yMax, this.config.yMin, this.elements.canvasBg.x, this.elements.canvasBg.toX, false, 0);
	...
	*/
	private function drawBars ()
	{
		...
		//Rendered Bar Count
		var renderedBarCount : Number = 0, renderedPBarCount:Number = 0, renderedNBarCount:Number = 0;
		...
		for (i = this.num; i >= 1; i --)
		{
			...
			for (j = 1; j <= this.numDS; j ++)
			{
				...
				if (this.dataset [j].data [i].isDefined)
				{
					...
					barIns = new Bar3D (barMC, this.dataset [j].data [i].x - this.config.basePlanePos + this.config.shifts.xShift, this.dataset [j].data [i].y, this.dataset [j].data [i].w, this.dataset [j].data [i].h, this.config.shifts.xShift, this.config.shifts.yShift, this.dataset [j].data [i].color, this.params.showPlotBorder, this.params.plotBorderColor, this.params.plotBorderAlpha);
					//Draw the bar
	...
	*/
	private function drawValues () : Void
	{
		...
		//Minimum text width for ellipses as we need to show at least one character and three dots for ellipses
		//we are wsing "W..." for calculating minimum value Label width.
		var minTextObj:Object = createText (true, "W...", this.tfTestMC, 1, 0, 0,0, valueStyleObj, false, 0,0);
		var minValueLabelWidth:Number =  minTextObj.width;
		//Delete simulation object
		minTextObj.tf.removeTextField();
		delete minTextObj;
		////Iterate through all bars
		for (i = 1; i <= this.numDS; i ++)
		{
			for (j = 1; j <= this.num; j ++)
			{
				...
				if (this.dataset [i].data [j].isDefined && this.dataset [i].data [j].showValue)
				{
					...
					if(this.dataset [i].data [j].displayValueDefined){
						valueObj = createText (false, this.dataset [i].data [j].displayValue, this.cMC, depth, xPos, this.dataset [i].data [j].y, 0, valueStyleObj, false, 0,0);
						if(valueObj.width > Math.abs(this.dataset [i].data [j].w)){
							var maxTextWidth:Number = Math.max(this.dataset [i].data [j].w, minValueLabelWidth);
							var valueLabelHeight:Number = valueObj.height;
							valueObj = createText (false, this.dataset [i].data [j].displayValue, this.cMC, depth, xPos, this.dataset [i].data [j].y, 0, valueStyleObj, true, maxTextWidth, valueLabelHeight);
						}
					}else{
		...
		if (this.params.showSum)
		{
			...
			for (i = 1; i <= this.num; i ++)
			{
				...
				if (this.sums [i].sum != 0)
				{
					...
					vAlign = "middle";
					//Get the y position
					xPos = this.config.basePlanePos + ((this.sums [i].sum < 0) ? (this.sums [i].nX - this.config.shifts.xShift - this.params.sumPadding) : (this.sums [i].pX + this.params.sumPadding));
					//Y-position

	...
	*/
	private function calculatePoints (isRedraw:Boolean)
	{
		...
		if(this.params.XTLabelManagement){
			...
			var newColumnWidth:Number = newBlockWidth / this.numDS;
			...
			if(diffColWidth > 0 && colWidth == this.params.maxColWidth ){
				newBlockWidth = newBlockWidth - (diffColWidth*this.numDS);
				newColumnWidth = actualColumnWidth;
				newBlockSpaceWidth += (diffColWidth*this.numDS*this.num)/(this.num + 1);
			}
		...
		for (i = 1; i <= this.num; i ++)
		{
			...
			{
				if(this.params.XTLabelManagement){
					this.dataset [j].data [i].x = this.elements.canvasBg.x + (amountOfDiff / 2) + (newBlockSpaceWidth * (i )) + (newBlockWidth * (i - 1)) + (newColumnWidth * (j - 0.5)) - this.config.shifts.xShift - this.params.canvasBgDepth/2;
				}else{

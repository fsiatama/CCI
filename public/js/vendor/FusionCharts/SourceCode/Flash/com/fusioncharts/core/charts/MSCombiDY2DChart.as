...
class com.fusioncharts.core.charts.MSCombiDY2DChart extends DualYAxis2DChart 
{
	...
	*/
	public function render (isRedraw:Boolean) : Void 
	{
		...
		} else 
		{
			...
			//as line or column both can be set in this chart the chart type for the label metrics class
			//need to be decided rightly.
	...
	*/
	private function returnDataAsObject (value : Number, color : String, displayValue:String, alpha : Number, ratio : String, toolText : String, link : String, showValue : Number, valuePosition:String, isDashed : Boolean, anchorSides : Number, anchorRadius : Number, anchorBorderColor : String, anchorBorderThickness : Number, anchorBgColor : String, anchorAlpha : Number, anchorBgAlpha : Number, isColumn:Boolean) : Object 
	{
		...
		if(isColumn){
			...
			dataObj.alpha = ColorExt.parseAlphaList (alpha.toString(), dataObj.color.length);
			...
			if (arrDocElement [i].nodeName.toUpperCase () == "GRAPH" || arrDocElement [i].nodeName.toUpperCase () == "CHART")
			{
				...
				for (j = 0; j < arrLevel1Nodes.length; j ++)
				{
					...
					} else if (arrLevel1Nodes [j].nodeName.toUpperCase () == "DATASET")
					{
						...
						if (this.params.plotGradientColor != "" && this.dataset [this.numDS].renderAs == "COLUMN")
						{
						...
						for (k = 0; k < arrLevel2Nodes.length; k ++)
						{
							if (arrLevel2Nodes [k].nodeName.toUpperCase () == "SET")
							{
								...
								this.dataset [this.numDS].data [setCount] = this.returnDataAsObject (setValue, setColor, setExDispVal,  setAlpha, setRatio, setToolText, setLink, setShowValue, setValuePosition, setDashed, setAnchorSides, setAnchorRadius, setAnchorBorderColor, setAnchorBorderThickness, setAnchorBgColor, setAnchorAlpha, setAnchorBgAlpha, isColumnDS);
							}
						}
	...
	*/
	private function parseAttributes (graphElement : XMLNode) : Void 
	{
		...
		this.params.sFormatNumberScale = toBoolean (getFN (atts ["sformatnumberscale"] , 1));
	...
	*/
	private function calculatePoints (isRedraw:Boolean)
	{
		...
		{
			// NRC - code block below changed to eliminate condition of isRedraw
			//Object to store dimensions
			var lgndDim : Object;
			//NRC - MC removed if present before recreation of the MC
			if(this.lgndMC){
			...
			for (i = 1; i <= this.num; i ++)
			{
				...
				for (j = 1; j <= this.numDS; j ++)
				{
					...
					if (this.dataset [j].renderAs == "COLUMN")
					{
						...
						//X-Position
						if(this.params.XTLabelManagement){
							columnWidth = newColumnWidth / this.numColDS;
							this.dataset [j].data [i].x = this.elements.canvas.x + (amountOfDiff/2) + (newSpaceWidth * i) + columnWidth * (colCounter - 0.5) + (columnWidth * this.numColDS * (i - 1));
						}else{
			...
			if (this.dataset [i].renderAs == "COLUMN")
			{
				for (j = 1; j <= this.num; j ++)
				{
					...
					if (this.dataset [i].data [j].isDefined)
					{
						...
						if (this.params.useRoundEdges){
							var colIns:RoundColumn2D = new RoundColumn2D(colMC, this.dataset [i].data [j].w-1, this.dataset [i].data [j].h, this.roundEdgeRadius, this.dataset [i].data [j].color[0].toString(16), this.dataset [i].plotBorderThickness, this.dataset [i].plotBorderAlpha);
						}else{				
							//Create column instance
							var colIns : Column2D = new Column2D (colMC, this.dataset [i].data [j].w, this.dataset [i].data [j].h, this.dataset [i].plotBorderColor, this.dataset [i].plotBorderAlpha, this.dataset [i].plotBorderThickness, this.dataset[i].data[j].color, this.dataset[i].data[j].alpha, this.dataset[i].data[j].ratio, this.params.plotFillAngle, false, this.dataset [i].data [j].dashed, this.params.plotBorderDashLen, this.params.plotBorderDashGap);
						}
						//Draw the column
						colIns.draw ();
	...
	*/
	public function reInit () : Void
	{
		...
		this.numAreaDS = 0;
		this.num = 0;
		//NRC - code changed: lgnd.reset() to lgnd.destroy(). [ reset() call was absent in the last NRC implementation though, but was present in ClickableLegend implementation]
		this.lgnd.destroy ();
		if (this.params.interactiveLegend){

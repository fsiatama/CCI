		...
		for (var labelsType in arrLabelsPlot) {
			...
			for (var i = 0; i<arrSubData.length; ++i) {
				...
				if (objLabels[labelsType][i]['showLabel']) {
					// joining line cosmetics according to xLabel, yLabel and tLabels.
					switch(labelsType)
					{
						case 'xLabels':
							// set xaxis tick color
							color = objLabels.tickLines['xAxisTickColor'];
							alpha = objLabels.tickLines['xAxisTickAlpha'];	
							thickness = objLabels.tickLines['xAxisTickThickness'];							
							break;
							
						case 'yLabels':
							// for y-axis tick, all properties will inherit from respective div lines .
							color = objLabels[labelsType][i]['yAxisTickColor'];
							alpha = objLabels[labelsType][i]['yAxisTickAlpha'];
							thickness = objLabels[labelsType][i]['yAxisTickThickness'];	
							break;							
						case 'tLabels':	
							// for trenline tick all properties will inherit from respective trend lines.
							color = objLabels[labelsType][i]['tLineTickColor'];
							alpha = objLabels[labelsType][i]['tLineTickAlpha'];
							thickness = objLabels[labelsType][i]['tLineTickThickness'];								
							break;
					}					
					// linestyle

 /**
* @class StackedColumn3DLineChart
* @author FusionCharts Technologies LLP, www.fusioncharts.com
* @version 3.2
*
* Copyright (C) FusionCharts Technologies LLP, 2010
*
* StackedColumn3DLineChart chart extends the SingleYAxis3DChart class to render a
...
class com.fusioncharts.core.charts.StackedColumn3DLineChart extends SingleYAxis3DChart
{
	...
	*/
	function StackedColumn3DLineChart (targetMC : MovieClip, depth : Number, width : Number, height : Number, x : Number, y : Number, debugMode : Boolean, lang : String, scaleMode:String, registerWithJS:Boolean, DOMId:String)
	{
		...
		this.log ("Chart Type", "Stacked 3D Column Line Chart", Logger.LEVEL.INFO);
		_version = getFCVersion();
		this.log ("Version", _version, Logger.LEVEL.INFO);
		//List Chart Objects and set them
		_arrObjects = new Array ("BACKGROUND", "CANVAS", "CAPTION", "SUBCAPTION", "YAXISNAME", "XAXISNAME", "DIVLINES", "YAXISVALUES", "DATALABELS", "DATAVALUES", "TRENDLINES", "TRENDVALUES",  "DATAPLOTCOLUMN", "DATAPLOTLINE", "TOOLTIP", "VLINES", "LEGEND", "VLINELABELS", "ANCHORS");
			...
			if (arrDocElement [i].nodeName.toUpperCase () == "GRAPH" || arrDocElement [i].nodeName.toUpperCase () == "CHART")
			{
				...
				for (j = 0; j < arrLevel1Nodes.length; j ++)
				{
					...
					} else if (arrLevel1Nodes [j].nodeName.toUpperCase () == "DATASET")
					{
						...
						this.dataset [this.numDS].color = formatColor (getFV (dsAtts ["color"] ,((this.dataset [this.numDS].renderAs == "LINE")? this.params.lineColor : undefined ), this.defColors.getColor ()));
						...
						for (k = 0; k < arrLevel2Nodes.length; k ++)
						{
							if (arrLevel2Nodes [k].nodeName.toUpperCase () == "SET")
							{
								...
								//Check if the dataset is column or line
								if (this.dataset [this.numDS].renderAs == "COLUMN")
								{
									//Store all these attributes as column data object.
									...
									this.dataset [this.numDS].data [setCount] = this.returnDataAsLineObject (setValue, setColor, setExDispVal, Number (setAlpha) , setToolText, setLink, setShowValue, setValuePosition, setDashed, setAnchorSides, setAnchorRadius, setAnchorBorderColor, setAnchorBorderThickness, setAnchorBgColor, setAnchorAlpha, setAnchorBgAlpha);
								}
	...
	*/
	private function calculatePoints (isRedraw:Boolean)
	{
		...
		for (i = 1; i <= this.num; i ++)
		{
			...
			{
				//If it's column dataset and value is defined
		...
		for (i = 1; i <= this.num; i ++)
		{
			...
			for (j = 1; j <= this.numDS; j ++)
			{
				//If it's a column dataset and is defined
				if (this.dataset [j].renderAs == "COLUMN")
				{
					if (this.dataset [j].data [i].isDefined)
					{

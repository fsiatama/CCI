...
//Legend Class
import com.fusioncharts.helper.ClickableLegend;
//Extensions
...
class com.fusioncharts.core.charts.Scatter2DChart extends XYPlotChart
{
	...
	*/
	function Scatter2DChart (targetMC : MovieClip, depth : Number, width : Number, height : Number, x : Number, y : Number, debugMode : Boolean, lang : String, scaleMode:String, registerWithJS:Boolean, DOMId:String)
	{
		...
		_arrObjects = new Array ("BACKGROUND", "CANVAS", "CAPTION", "SUBCAPTION", "YAXISNAME", "XAXISNAME", "DIVLINES", "VDIVLINES", "VLINES", "YAXISVALUES", "XAXISVALUES", "HGRID", "VGRID", "DATALABELS", "DATAVALUES", "TRENDLINES", "VTRENDLINES", "REGRESSIONLINE", "TRENDVALUES", "DATAPLOT", "ANCHORS", "TOOLTIP", "LEGEND", "QUADRANTLINE", "QUADRANTLABELS");
		super.setChartObjects (_arrObjects);
		//Configuration whether we've to draw any line
		this.config.drawLine = false;
		//Store configuration that it's not a bubble chart
		this.config.isBubbleChart = false;
	...
	*/
	public function render (isRedraw:Boolean) : Void
	{
		...
		} else
		{
			...
			//Update timer
			this.timeElapsed += (this.params.animation) ? this.styleM.getMaxAnimationTime (this.objects.VLINES, this.objects.TRENDLINES, this.objects.VTRENDLINES, this.objects.TRENDVALUES, this.objects.REGRESSIONLINE) : 0;
	...
	*/
	private function parseXML () : Void
	{
			...
			if (arrDocElement [i].nodeName.toUpperCase () == "GRAPH" || arrDocElement [i].nodeName.toUpperCase () == "CHART")
			{
				...
				for (j = 0; j < arrLevel1Nodes.length; j ++)
				{
					...
					} else if (arrLevel1Nodes [j].nodeName.toUpperCase () == "DATASET")
					{
						...
						this.dataset [this.numDS].anchorBorderColor = formatColor (getFV (dsAtts ["anchorbordercolor"] , dsAtts ["color"], this.params.anchorBorderColor, this.dataset [this.numDS].color));
						...
						this.dataset [this.numDS].anchorBgAlpha = getFN (dsAtts ["anchorbgalpha"] , dsAtts ["alpha"], this.params.anchorBgAlpha);
						this.dataset [this.numDS].anchorAlpha = getFN (dsAtts ["anchoralpha"] , dsAtts ["anchorbgalpha"], dsAtts ["alpha"], this.params.anchorAlpha);
						...
						this.dataset [this.numDS].regressionLineAlpha = getFN (dsAtts ["regressionlinealpha"] , dsAtts ["alpha"], this.params.regressionLineAlpha);
						//Data set line properties
						this.dataset [this.numDS].drawLine = toBoolean (getFN (dsAtts ["drawline"] , this.params.drawLines, (dsAtts ["linealpha"] > 0) ? 1 : 0));
						...
						//Extract Properties
						this.dataset [this.numDS].lineColor = formatColor (getFV (dsAtts ["linecolor"] , this.params.lineColor, this.dataset [this.numDS].color));
						this.dataset [this.numDS].lineThickness = getFN (dsAtts ["linethickness"] , this.params.lineThickness);
						this.dataset [this.numDS].lineAlpha = getFN (dsAtts ["linealpha"] , dsAtts ["alpha"], this.params.lineAlpha);
						...
						var isLineVisible:Boolean = (this.dataset [this.numDS].drawLine && this.dataset [this.numDS].lineAlpha > 0)? true:false;
						//Whether anchors are visible
						var isAnchorVisible:Boolean = (this.dataset [this.numDS].drawAnchors && this.dataset [this.numDS].anchorAlpha > 0 && this.dataset [this.numDS].anchorRadius > 0 && this.dataset [this.numDS].anchorSides > 2)? true:false;
						//Consequtive plot visible
						var isConsequtivePlotVisible:Boolean = false;
						//Previuous plot visible
						var previousPlotVisible:Boolean = false;
						//Any vaild plot shown
						var anyValidPlot:Boolean = false;
						for (k = 0; k < arrLevel2Nodes.length; k ++)
						{
							if (arrLevel2Nodes [k].nodeName.toUpperCase () == "SET")
							{
								...
								//Whether valid plot or not
								if(!anyValidPlot){
									anyValidPlot = ((!isNaN(setYV) && this.dataset [this.numDS].anchorAlpha > 0) || (isAnchorVisible || setShowValue == 1))? true : anyValidPlot;
								}
								//Whether two or more consequtive plot visible
								if(!isConsequtivePlotVisible && isLineVisible && !isNaN(setYV) && previousPlotVisible){
									isConsequtivePlotVisible = true;
								}
								//Reset previous plot visible
								previousPlotVisible = !isNaN(setYV);
						...
						if(this.dataset [this.numDS].includeInLegend){
							this.dataset [this.numDS].includeInLegend = (isConsequtivePlotVisible || anyValidPlot)? true:false;
						}
		...
		if(!isNaN(this.params.xAxisMinValue) && (!isMinValueSameWithCategory) && this.params.showVLimits && this.params.xAxisLabelMode != "auto" && this.numCat > 0){
			...
			this.categories[this.numCat].catLineValues = 0;
			this.categories[this.numCat].catLabelValues = String(this.params.xAxisMinValue);
		}
		...
		if(!isNaN(this.params.xAxisMaxValue) && (!isMaxValueSameWithCategory) && this.params.showVLimits && this.params.xAxisLabelMode != "auto" && this.numCat > 0){
			...
			this.categories[this.numCat].catLineValues = 0;
			this.categories[this.numCat].catLabelValues = String(this.params.xAxisMaxValue);
		}
	...
	*/
	private function parseAttributes (graphElement : XMLNode) : Void
	{
		...
		//Line Properties
		this.params.drawLines = toBoolean (getFN (atts ["drawlines"] , 0));
		...
		//For regression line
		this.params.regressionLineColor = formatColor (getFV (atts ["regressionlinecolor"] , this.params.anchorBorderColor));
		this.params.regressionLineThickness = getFN (atts ["regressionlinethickness"] , this.params.linethickness);
		this.params.regressionLineAlpha = getFN (atts ["regressionlinealpha"] , this.params.anchorAlpha);
		...
		*/
		...
		for (m = 1; m <= this.numDS; m ++)
		{
			...
			if (this.dataset [m].drawLine)
			{
				...
				//Register with object Manager
				objM.register(holderMC, "ChartHolder_" + m, "ChartHolder_" + m)
			...
			if (this.dataset [i].drawAnchors)
			{
				for (j = 1; j <= this.dataset [i].num; j ++)
				{
					...
					if (this.dataset [i].data [j].isDefined)
					{
						...
						if(this.dataset [i].anchorSides < 3){
							//Draw the circle
							DrawingExt.drawCircle(anchorMC, 0, 0, this.dataset [i].anchorRadius, this.dataset [i].anchorRadius, 0, 360)
						} else {
							//Draw the polygon
						...
						//Register with object Manager
						objM.register(anchorMC, "DATAPLOT_" + i + "_" + j, "DATAPLOTS_"+i)
						//Apply animation
						if (this.params.animation)
						{
	...
	*/
	private function legendClick(target:Object):Void{
		if (this.chartDrawn){
			...
			objM.toggleGroupVisibility("ChartHolder_"+target.index, target.active);

...
class com.fusioncharts.core.chartobjects.Doughnut2D {
	 ...
	 */
	public function Doughnut2D(chartClassRef, mcPie:MovieClip, obj:Object, canvasXMin:Number, canvasXMax:Number) {
	...
	private function drawDashedBorder(_mc:MovieClip):Void {
		...
		for (var i:Number = 1; i<=loopsLine; ++i) {
			...
			//Fix for dash going outside the doughnut [FCXT-227]
			//When end position distance from center is more than radius draw upto radius.
			if(Math.abs(xcenter - endX) > radius || Math.abs(ycenter - endY) > radius){
				endX = xcenter+ radius*Math.cos(startAng);
				endY = ycenter+ radius * Math.sin(startAng);
			}
			//
			mcCanvas.lineTo(endX, endY);
		}
		//Fix for dash going outside the doughnut [FCXT-227]
		//Calculating start and end  position of the outer curve
		var outerCurveStartX :Number = xcenter+radius*Math.cos(startAng);
		var outerCurveStartY :Number = ycenter+radius*Math.sin(startAng);
		var outerCurveEndX :Number = xcenter+radius*Math.cos(endAng);
		var outerCurveEndY :Number = ycenter+radius*Math.sin(endAng);
		...
		for (var i:Number = 1; i<=loops; ++i) {
			...
			//Fix for dash going outside the doughnut [FCXT-227]
			//For the last pie the angle should not more than 360.
			if(Math.floor(MathExt.toDegrees(endAng)) == 0 && MathExt.toDegrees(startAng+i*outerDashAng) >= 360){
				mcCanvas.curveTo(controlX, controlY, outerCurveEndX, outerCurveEndY);
				break;
			//For other pies except the last one if dash is going more than end angle ignore it.
			}else if((MathExt.toDegrees((startAng+i*outerDashAng)) > MathExt.toDegrees(endAng)) && Math.floor(MathExt.toDegrees(endAng)) != 0){
				break;
			}else{
				mcCanvas.curveTo(controlX, controlY, endX, endY);
			}
		}
		//Fix for dash going outside the pie [FCXT-227]	
		//Move to curve end position.	
		mcCanvas.moveTo(outerCurveEndX, outerCurveEndY);
		...
		for (var i:Number = 1; i<=loops1; ++i) {
			...
			var controlX:Number = roundOff(xcenter+e*Math.cos(b));
			var controlY:Number = roundOff(ycenter+e*Math.sin(b));
			//Fix for dash going inside the doughnut [FCXT-227]
			//For the last pie the angle should not more than 360.
			if(Math.floor(MathExt.toDegrees(endAng)) == 0 && MathExt.toDegrees(startAng+i*innerDashAng) >= 360){
				mcCanvas.curveTo(controlX, controlY, xInEnd, yInEnd);
				break;
			//For other pies except the last one if dash is going more than end angle ignore it.
			}else if((MathExt.toDegrees((startAng+i*innerDashAng)) > MathExt.toDegrees(endAng)) && Math.floor(MathExt.toDegrees(endAng)) != 0){
				break;
			}else{
				mcCanvas.curveTo(controlX, controlY, endX, endY);
			}

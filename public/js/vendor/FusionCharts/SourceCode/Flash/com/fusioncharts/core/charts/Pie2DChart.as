...
class com.fusioncharts.core.charts.Pie2DChart extends Chart {
	...
	//
	//Map of Pie2D objects with their respective entries in this.data
	...
	*/
	 ...
	 */
	private function checkResizePendingForChartInit(prop:String, oldVal:Boolean, newVal:Boolean, insRef:Pie2DChart):Boolean{
	 ...
	 * @returns				the new value
	 */
	private function checkResizePendingForSlicing (prop:String, oldVal:Number, newVal:Number, insRef:Pie2DChart):Number{
				...
				for (var j = 0; j<arrLevel1Nodes.length; j++) {
					...
					if (arrLevel1Nodes[j].nodeName.toUpperCase() == "SET") {
						...
						if ((setValue != 0 || this.params.showZeroPies) && !isNaN(setValue) && setFillAlpha != 0) {
							...
							this.data[this.num-1] = this.returnDataAsObject(setLabel, setShowLabel, setValue, setShowValue, setColor, color, setExDispVal, setBorderColor, setFillAlpha, setBorderAlpha, setToolText, setLink, setIsSliced, setIsDashed);
						}
	...
	*/
	private function parseAttributes(graphElement:XMLNode):Void {
		...
		this.params.pieBorderAlpha = getFN(atts["plotborderalpha"], atts["pieborderalpha"], 80);
	 ...
	 */
	private function returnDataAsObject(dataLabel:String, showLabel:Boolean, dataValue:Number, showValue:Boolean, color:Number, hexColor:String, displayValue:String, bordercolor:Number, fillAlpha:Number, borderAlpha:Number, toolText:String, link:String, isSliced:Boolean, isDashed:Boolean):Object {
		//Create a container
	...
	private function calculateForWidthAndHeight(isRedraw:Boolean):Void{
		...
		if (this.params.showLegend)
		{
			...
			if(!isRedraw){
				...
				//Setting the 'items' array of the current legend to that of previous one
	...
	private function draw(isRedraw:Boolean):Void {
		...
		if(!isRedraw){
			...
			if (this.config.totalSlices>1) {
				// call function to set watch on chart initialisation end; handles all cases except for singleton
				this.setInitWatch();
			}
	...
	*/
		...
		if (this.config.totalSlices>1) {
			...
			for (var i:Number = 0; i<this.config.totalSlices; ++i) {
				...
				for (var t:Number = 0; t<arrMcName.length; ++t) {
					...
					// storing the string id by which the Pie2d instance is stored in config.objPie2D, mapped to the index of the pie data in this.data
					...
					this.config.objPie2D[strName] = new Pie2D(this, mcMain, objProps, this.plotX, this.plotX + this.plotWidth);
			...
			this.config.objPie2D.pie = new Pie2D(this, mcMain, objProps, this.plotX, this.plotX + this.plotWidth);
			// method called to set mouse event listener for this Pie2D instance
			...
			if (tracker>=6) {
				...
				if(insRef.params.manageLabelOverflow){
					...
					if(_mc.labelTxt != undefined){
						var strText:String =  _mc.labelTxt;
						//Replace {br}, {BR} with new line break
						...
						strText = StringExt.replace(strText, "<br>", "\n");
						strText = StringExt.replace(strText, "&lt;br&gt;", "\n");
						_mc.labelText =  strText;
						_mc.mcLabel.label_txt.text = _mc.labelTxt;
					}
				} else {
					var strText:String =  _mc.store.labelText;
					//Replace {br}, {BR} with new line break
					strText = StringExt.replace(strText, "{BR}", "\n");
					strText = StringExt.replace(strText, "{br}", "\n");
					//Also convert <BR> to /n.
	...
	private function setEventHandlers(_mc:MovieClip, objProps:Object, Pie2DRef):Void {
			...
			if (!(this.config.enableRotation || this.config.enableLinks || this.config.totalSlices == 0)) {
				...
				_mc.onRelease = function(){
					...
					// else if legend is not there in the chart	
						...
						// ask the Pie2D instance to move the pie 
						Pie2DInsRef.movePie();
					}
				}
			}
		...
		if(this.config.totalSlices > 1){
			//Add listener for "slicing" event from Pie2D objects for slice movement start or end.
			Pie2DInsRef.addEventListener('slicing', this);
		}
	}
	...
	private function sliceViaJS(id:Number):Void{
		...
		if(this.config.totalSlices > 1 && !this.rotating && this.chartInit){
			...
			// else if legend is not there in the chart	
				...
				// get the reference of the Pie2D instance for the pie
				var Pie2DInsRef:Pie2D = this.config.objPie2D[this.map[id]];
	 ...
	 */
	private function enableSlicingViaJS(obj, item) {
		if(this.numSlicing != 0 || this.rotating){
			return;
		}
		cmiRotation.enabled = true;
	...
	*/
	private function legendClick(target:Object):Void{
		...
		if (this.config.totalSlices > 1 && this.chartDrawn) {
			...
			// slice/unslice the pie
			this.config.objPie2D[this.map[target.index]].movePie();

...
class com.fusioncharts.engine3D.Model {
	...
	public static function getArea3DBlocks(arrH:Array, zOrigin:Number, d:Number):Array {
		...
		//To hold the sign of last non-zero data value, after last x-interception
		var lastSign:Number = null;
		...
		for (var i = 0; i < arrPoints.length - 1; ++i) {
			//Checking x-interception for/with consecutive multiple zero data values
			if(lastSign && lastSign * arrPoints[i+1][1] < 0 && arrPoints[i][1] == 0){
				arrIntercept.push([i, null]);
				lastSign = null;
			//if consecutive heights are of opposite sign
			} else if (arrPoints[i + 1][1] * arrPoints[i][1] < 0) {
				...
				lastSign = null;
			//Checking x-inteception for zero data value and its two nearest neighbours on oppposite sides
				...
				lastSign = null;
			//Setting the sign of current data value for subsequent checking for x-interception
			} else if(i>0 && i < arrPoints.length - 1 && arrPoints[i][1] != 0 && !isNaN(arrPoints[i][1])){
				lastSign = Math.abs(arrPoints[i][1]) / arrPoints[i][1];
			}
		}
		...
		for (var i = 0; i <= arrIntercept.length; ++i) {
			...
			// starting x value for the next block
			x = arrIntercept[i][1];

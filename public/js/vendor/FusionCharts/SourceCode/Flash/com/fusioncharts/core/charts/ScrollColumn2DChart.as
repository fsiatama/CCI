	...
	*/
	private function calculatePoints (isRedraw:Boolean)
	{
		...
		if (this.params.labelDisplay=="AUTO"){
			...
			var canvasTotalWidth:Number = canvasWidth*((this.num*this.numDS)/this.params.numVisiblePlot);
		...
		if(!this.scrollRequired){
			...
			if (this.params.labelDisplay == "ROTATE")
			{
				...
				var maxLabelHeight : Number = (canvasWidth*((this.num*this.numDS)/this.params.numVisiblePlot)/ finalVisibleCount);
		...
		if(this.params.numVisiblePlot % this.numDS == 0){
			numSpacesVisible = (this.params.numVisiblePlot/this.numDS) + 1;
			unitSpaceWidth = visibleSpaceWidth/numSpacesVisible;
		} else {
			numSpacesVisible = Math.ceil(this.params.numVisiblePlot/this.numDS);
			unitSpaceWidth = visibleSpaceWidth/numSpacesVisible;
		}
		...
		this.config.blockWidth = columnWidth * this.numDS;
		...
		if(this.scrollRequired){
			var panelMinWidth:Number = this.config.interBlockSpace + this.config.blockWidth;
		...
		if(this.params.XTLabelManagement && !this.scrollRequired){
			...
			var newColmnWidth:Number = newBlockWidth / this.numDS
		...
		for (i = 1; i <= this.num; i ++)
		{
			...
			if(this.params.XTLabelManagement && !this.scrollRequired){
				this.categories [i].x = (newSpaceWidth * i) + (newColmnWidth * this.numDS * (i - 0.5));
			}else{
				this.categories [i].x = (this.config.interBlockSpace * i) + (columnWidth * this.numDS * (i - 0.5));
			}
			for (j = 1; j <= this.numDS; j ++)
			{
				//X-Position
				if(this.params.XTLabelManagement&& !this.scrollRequired){
					this.dataset [j].data [i].x = (newSpaceWidth * i) + newColmnWidth * (j - 0.5) + (newColmnWidth * this.numDS * (i - 1));
				}else{
					this.dataset [j].data [i].x = (this.config.interBlockSpace * i) + columnWidth * (j - 0.5) + (columnWidth * this.numDS * (i - 1));
				}
				//Set the y position

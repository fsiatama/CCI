...
*
* Pareto3DChart chart extends the DualYAxis3DChart class to render a
* 3D Pareto Chart.
*/
//Import Chart class
...
class com.fusioncharts.core.charts.Pareto3DChart extends DualYAxis3DChart 
{
	...
	*/
	function Pareto3DChart (targetMC : MovieClip, depth : Number, width : Number, height : Number, x : Number, y : Number, debugMode : Boolean, lang : String, scaleMode:String, registerWithJS:Boolean, DOMId:String)
	{
		...
		this.log ("Chart Type", "3D Pareto Chart", Logger.LEVEL.INFO);
		_version = getFCVersion();
		this.log ("Version", _version, Logger.LEVEL.INFO);
		//List Chart Objects and set them
		_arrObjects = new Array ("BACKGROUND", "CANVAS", "CAPTION", "SUBCAPTION", "YAXISNAME", "XAXISNAME", "DIVLINES", "YAXISVALUES", "DATALABELS", "DATAVALUES", "TRENDLINES", "TRENDVALUES", "DATAPLOTCOLUMN", "DATAPLOTLINE", "ANCHORS", "TOOLTIP", "VLINES", "VLINELABELS");
	...
	*/
	public function render (isRedraw:Boolean) : Void 
	{
		...
		} else 
		{
			...
			//Update timer
			this.timeElapsed += (this.params.animation) ? this.styleM.getMaxAnimationTime (this.objects.DATALABELS, this.objects.DATAPLOTCOLUMN) : 0;
	...
	*/
	private function calculatePoints ()
	{
		...
		var colSpace : Number = ((this.elements.canvasBg.w - 2 * this.params.canvasPadding) - (blockWidth * this.num)) / (this.num + 1);
		...
		for (i = 1; i <= this.num; i ++)
		{
			...
			if(this.params.XTLabelManagement){
				this.categories [i].x = this.elements.canvasBg.x + (amountOfDiff/2) + (newSpaceWidth * i) + ((i - 0.5) * newBlockWidth) - this.config.shifts.xShift - this.params.canvasBgDepth/2;
			}else{
				this.categories [i].x = this.elements.canvasBg.x + this.params.canvasPadding + (colSpace * i) + (blockWidth * (i - 0.5)) - this.config.shifts.xShift;
			}
			...
			for (j = 1; j <= this.numDS; j ++)
			{
				if (this.dataset [j].parentYAxis == "P")
				{
					...
					if(this.params.XTLabelManagement){
						this.dataset [j].data [i].x = this.elements.canvasBg.x + (amountOfDiff / 2) + (newSpaceWidth * i) + (newBlockWidth * (i - 1)) + (newColumnWidth * (colCounter - 0.5)) - this.config.shifts.xShift - this.params.canvasBgDepth/2;
					}else{
						this.dataset [j].data [i].x = this.elements.canvasBg.x + this.params.canvasPadding + (colSpace * i) + (blockWidth * (i - 1)) + (oColWidth * (colCounter - 0.5)) - this.config.shifts.xShift;
					}
					//Set the y position
	...
	*/
	private function drawHeaders ()
	{
		...
		if (this.params.xAxisName != "")
		{
			...
			var xAxisNameObj : Object = createText (false, this.params.xAxisName, this.cMC, this.dm.getDepth ("XAXISNAME") , this.elements.canvasBase.x + (this.elements.canvasBase.w / 2) , this.height -(this.params.chartBottomMargin + this.elements.xAxisName.h + this.params.xAxisNamePadding), 0, xAxisNameStyleObj, true, this.elements.xAxisName.w, this.elements.xAxisName.h);
			//Apply animation
			if (this.params.animation)
			{
				this.styleM.applyAnimation (xAxisNameObj.tf, this.objects.XAXISNAME, this.macro, this.elements.canvasBase.x + (this.elements.canvasBase.w / 2) - (this.elements.subCaption.w / 2) , 0, this.height -(this.params.chartBottomMargin + this.elements.xAxisName.h + this.params.xAxisNamePadding), 0, 100, null, null, null);
			}
			//Apply filters
	...
	*/
	private function drawAnchors () : Void 
	{
		...
		{
			//If it's line dataset
			if (this.params.drawAnchors && this.dataset [i].parentYAxis == "S")
			{
				for (j = 1; j <= this.num; j ++)
				{
					//If defined

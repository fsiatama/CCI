...
class com.fusioncharts.core.charts.StackedArea2DChart extends SingleYAxis2DVerticalChart 
{
	...
	*/
	private function calculatePoints (isRedraw:Boolean)
	{
		...
		if(this.params.XTLabelManagement){
			var interPointWidth : Number = (metricsObj.totalInterPlotWidth) / (this.num - 1);
		}else{
	...
	private function drawNormalLabels ()
	{
		...
		for (i = 1; i <= this.num; i ++)
		{
			...
			if (this.categories [i].showLabel)
			{
				...
				if (this.num>1 && count==this.config.finalVisibleCount){					
					//If in wrap mode, check width
					if (this.params.labelDisplay == "WRAP"){
						if ((this.categories [i].x+labelWidth)>(this.width-this.params.chartRightMargin)){												
							//Allot max available space available between this and previous label.
							labelWidth = (this.categories [i].x - this.categories [lastShowIndex].x)/2;
							//Set alignment to right
							align = "right";
						}
					}
					else if (this.params.labelDisplay == "ROTATE"){
						//If in rotate mode, check height (which, in effect is width in rotated mode)
						//If it's going out of canvas, then take care of it.
						if ((this.categories [i].x+(labelHeight/2))>(this.width)){
							//Allot max available space available between this and previous label.
							labelHeight = (this.categories [i].x - this.categories [lastShowIndex].x);
						}
					}
				}
				...
				if (this.params.labelDisplay == "ROTATE")
				{
					...
					//Create text box and get height
					labelObj = createText (false, this.categories [i].label, this.cMC, depth, this.categories [i].x, this.elements.canvas.toY + this.params.labelPadding, this.config.labelAngle, labelStyleObj, true, this.config.wrapLabelWidth, labelHeight);
				} else if (this.params.labelDisplay == "WRAP")
				{
					//Case 2 (WRAP)
					//Set alignment
					...
					labelStyleObj.vAlign = "bottom";
					labelObj = createText (false, this.categories [i].label, this.cMC, depth, this.categories [i].x, this.elements.canvas.toY + this.params.labelPadding, 0, labelStyleObj, true, labelWidth, this.config.wrapLabelHeight);

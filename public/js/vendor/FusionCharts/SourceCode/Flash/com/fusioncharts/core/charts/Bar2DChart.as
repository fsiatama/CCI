...
class com.fusioncharts.core.charts.Bar2DChart extends SingleYAxis2DHorizontalChart 
{
	...
	*/
	private function calculatePoints ()
	{
		...
		for (i = 1; i <= this.num; i ++)
		{
			...
			//Update counter
			finalVisibleCount = (this.data [i].showLabel && this.data[i].label!="") ? (finalVisibleCount + 1) : (finalVisibleCount);
		}
		...
		if (finalVisibleCount>0){
			...
			if (maxHorLabelWidth<=maxWidthAllocation){
				...
				for (i=startLabelWidthRatio; i<=endLabelWidthRatio; i=i+5){
					...
					{
						//If the label is to be shown
						if (this.data [j].showLabel)
						{
							//Create text box with specified width and get height
							labelObj = createText (true, this.data [j].label, this.tfTestMC, 1, testTFX, testTFY, 0, labelStyleObj, true, simulatedLabelWidth, canvasHeight);
				...
				if (finalVisibleCount>0 && (this.config.labelAreaHeight==0 || this.config.labelAreaWidth==0)){
					//Set max label width to maximum
		...
		//Now, calculate the spacing on canvas and individual bar height
	...
	*/
	private function drawHeaders ()
	{
		...
		if (this.params.yAxisName != "")
		{
			...
			var yAxisNameObj : Object = createText (false, this.params.yAxisName, this.cMC, this.dm.getDepth ("YAXISNAME") , this.width/2 , this.height - this.params.chartBottomMargin, 0, yAxisNameStyleObj, true, this.width - (this.params.chartLeftMargin+this.params.chartRightMargin), this.elements.yAxisName.h);
			//Apply animation
			if (this.params.animation)
			{
	...
	*/
	public function reInit () : Void
	{
		...
		//Whether negative number present
		this.config.negativePresent = false;
		//Reset defaultPlotSpacePercent to false
		this.defaultPlotSpacePercent = false;
	}

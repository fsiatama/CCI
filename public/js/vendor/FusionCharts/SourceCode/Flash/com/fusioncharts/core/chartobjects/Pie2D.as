...
class com.fusioncharts.core.chartobjects.Pie2D {
	...
	private function drawDashedBorder(_mc:MovieClip):Void {
		...
		//Fix for dash going outside the pie [FCXT-227]
		//Calculating start and end  position of the curve
		var curveStartX :Number = xcenter+radius*Math.cos(startAng);
		var curveStartY :Number = ycenter+radius*Math.sin(startAng);
		var curveEndX :Number = xcenter+radius*Math.cos(endAng);
		var curveEndY :Number = ycenter+radius*Math.sin(endAng);
		//Move to curve start position
		mcCanvas.moveTo(curveStartX, curveStartY);
		...
		for (var i:Number = 1; i<=loops; ++i) {
			...
			if(Math.floor(MathExt.toDegrees(endAng)) == 0 && MathExt.toDegrees(startAng+i*dashAng) >= 360){
				mcCanvas.curveTo(controlX, controlY, curveEndX, curveEndY);
				break;
			//For other pies except the last one if dash is going more than end angle ignore it.
			}else if((MathExt.toDegrees((startAng+i*dashAng)) > MathExt.toDegrees(endAng)) && Math.floor(MathExt.toDegrees(endAng)) != 0){
				break;
			}else{
		...
		mcCanvas.moveTo(curveEndX, curveEndY);

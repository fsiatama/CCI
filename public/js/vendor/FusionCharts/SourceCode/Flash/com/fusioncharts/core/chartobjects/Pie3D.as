...
class com.fusioncharts.core.chartobjects.Pie3D {
	...
	// stores the reference of MathExt.toNearestTwip()
	 ...
	 */
	public function Pie3D(chartClassRef, mcTarget:MovieClip, mcPie:MovieClip, obj:Object, canvasXMin:Number, canvasXMax:Number) {
	...
	private function drawLabel():Void {
		...
		var xSend:Number = arr['labelProps'][0];
		var ySend:Number = arr['labelProps'][1];
		var quadrantId:Number = arr['labelProps'][2];
		var isIcon:Boolean = (arr['labelProps'][3] == 'icon') ? true : false;
		
		// get the type of label - this will decide the wordwrap/ellipses and auto aligning properties
		var type:String = arr['labelProps'][(arr['labelProps'].length-1)];
		var wrapSpaceObj:Number = arr['labelProps']['wrapSpace'];
		var maxYlimit:Number = arr['labelProps']['maxYlimit'];
		
		// to avoid double labelling for conjugated pies
		if (arr['isLabelInvisible']) {
			_mc._visible = false;
		}
		//---------------------------------------------                                         
		// text for display in label
		var txt:String = arr['labelText'];
			...
			if(manageLabelOverflow){
				...
				if (objData.totalSlices>1) {
					...
					if(isOverflow){
						...
						//now go for ellipses instead of wordWrap when quadrant is over crowded and the 
	...
	public function movePie():Void {
		...
		// an extra negative sign is added because the pie movieclips are inverted
		...
		_mc.mcAnimCTRL.onEnterFrame = function(){
			thisRef.movePieCallback(mcToBe, dx, dy);
		}
	...
	private function movePieCallback(_mc:MovieClip, dx:Number, dy:Number):Void {
		...
		if (_mc.tracker<20) {
			...
			if (objData.isSmartLabels) {
				...
				//-------------------------------------
				// smartline redrawing process w.r.t. smartline coordinates retrieved ... to keep the vertical placement of the labels constant
				_mc.mcLabel.clear();
				//smartline coordinates retrieved
	...
	* maximum text to it.And add ellipses if the text overflows.At present this is 

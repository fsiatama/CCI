...
class com.fusioncharts.core.SingleYAxisChart extends Chart 
{
	...
	//Divline count modified or not(For drag charts)
	private var divlineModified:Boolean = false;
	/**
	...
	*/
	private function getAxisLimits (maxValue : Number, minValue : Number, stopMaxAtZero : Boolean, setMinAsZero : Boolean) : Void 
	{
		...
		//Unable to compare maxValue using logical operator for stacked charts only. so, parseInt used. Issue ID FCXT-374.
		if (this.config.yMaxGiven == false || (this.config.yMaxGiven == true && (Number(this.params.yAxisMaxValue) < parseInt((maxValue).toString(),10)) && isMaxValid))
		{
	...
	private function calcDivs () : Void 
	{
		...
		*/
		...
		if (this.config.zeroPRequired == true && this.config.zeroPIncluded == false)
		{
			...
			//Set a flag which indicates that divline increased due to zeroplane (For Drag chart limit modification #PCXT-324);
			this.divlineModified = true;
		}
	...
	*/
	private function returnDataAsDivLine (value : Number) : Object 
	{
		...
		//Form Display value - number formatting
	...
	*/
	private function adjustYAxisLabelStep(maxItemDimension:Number, availableDimension:Number):Void{
		...
		if (numDiv>0){
			if ((availableDimension/numDiv)<maxItemDimension){
				//Calculate how many labels can fit in well
				var numFitLabels = (availableDimension/maxItemDimension);
				//Set the step to ceiling value. Allows for some spacing.
			...
			//Re-set the individual display property for each div-line
			var count : Number = 0;
			while (count <= numDiv)
			{
				//Based on yAxisValueStep, we need to hide required div line values
				...
				//If the div-value is 0 and zero plane value has to be shown, set to true
				if (this.divLines[count].value==0){
					this.divLines [count].showValue = this.params.showZeroPlaneValue;
				}
				//Increment counter
				count ++;
			}

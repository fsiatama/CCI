...
class com.fusioncharts.core.charts.Pie3DChart extends Chart {
	...
	*/
	 ...
	 */
	private function checkResizePendingForChartInit(prop:String, oldVal:Boolean, newVal:Boolean, insRef:Pie3DChart):Boolean{
	 ...
	 * @returns				the new value
	 */
	private function checkResizePendingForDimensionChange(prop:String, oldVal:Boolean, newVal:Boolean, insRef:Pie3DChart):Boolean{
	 ...
	 * @returns				the new value
	 */
	private function checkResizePendingForSlicing (prop:String, oldVal:Number, newVal:Number, insRef:Pie3DChart):Number{
	...
	*/
	...
	private function cleanUp():Void {
		...
		for (var p in this.config.objPie3D) {
			this.config.objPie3D[p].removeEventListener('slicing', this);
				...
				for (var j = 0; j<arrLevel1Nodes.length; j++) {
					...
					if (arrLevel1Nodes[j].nodeName.toUpperCase() == "SET") {
						...
						if ((setValue != 0 || this.params.showZeroPies) && !isNaN(setValue)) {
							...
							var setShowLabel:Boolean = toBoolean(getFN(atts["showlabel"], this.params.showNames));
							var setShowValue:Boolean = toBoolean(getFN(atts["showvalue"], this.params.showValues));
							...
							this.data[this.num-1] = this.returnDataAsObject(setLabel, setShowLabel, setValue, setShowValue, setColor, color, setExDispVal,  setBorderColor, setToolText, setLink, setIsSliced);
						}
	...
	*/
	private function parseAttributes(graphElement:XMLNode):Void {
		...
		//Validate pieYScale value
		this.params.pieYScale = (this.params.pieYScale < 30)? 30 : ((this.params.pieYScale > 80)? 80 : this.params.pieYScale);
	 ...
	 */
	private function returnDataAsObject(dataLabel:String, showLabel:Boolean, dataValue:Number, showValue:Boolean, color:Number, hexColor:String, displayValue:String, bordercolor:Number, toolText:String, link:String, isSliced:Boolean):Object {
		//Create a container
		var dataObj:Object = new Object();
		//Store the values
		dataObj.label = dataLabel;
		dataObj.showLabel = showLabel;
	...
	*/
	private function calculatePoints():Void {
		...
		for (var i:Number = 0; i<this.num; i++) {
			//Format and store
			this.data[i].displayValue = formatNumber(this.data[i].value, this.params.formatNumber, this.params.decimals, this.params.forceDecimals, this.params.formatNumberScale, this.params.defaultNumberScale, this.config.nsv, this.config.nsu, this.params.numberPrefix, this.params.numberSuffix, this.params.scaleRecursively, this.params.scaleSeparator, this.params.maxScaleRecursion);
			this.data[i].percentValue = formatNumber((this.data[i].value/this.config.sumOfValues)*100, this.params.formatNumber, this.params.decimals, this.params.forceDecimals, false, this.params.defaultNumberScale, this.config.nsv, this.config.nsu, "", "%", this.params.scaleRecursively, this.params.scaleSeparator, this.params.maxScaleRecursion);
			...
			var strLabel:String = '';
			if (this.data[i].showLabel && this.data[i].label != '') {
	...
	private function calculateForWidthAndHeight(isRedraw:Boolean):Void{
		...
		if (this.params.showLegend)
		{
			...
			var interactive:Boolean = (this.num > 1)? (this.params.interactiveLegend  && (this.params.clickURL == "" || this.params.clickURL == null || this.params.clickURL == undefined )): false;
			...
			if(!isRedraw){
				...
				//reset previous legend 'items' array to new one
	...
	*/
	...
	private function drawChart(isGetting2DRadius:Boolean):Void {
		...
		if (this.config.totalSlices>1) {
			...
			for (var i:Number = 0; i<this.config.totalSlices; ++i) {
				...
				// storing the string id by which the Pie3d instance is stored in config.objPie3D, mapped to the index of the pie data in this.data
				...
				this.config.objPie3D[strName] = new Pie3D(this, mcPieH, mcMain, objProps, this.plotX, this.plotX + this.plotWidth);
			...
			this.config.objPie3D.pie = new Pie3D(this, mcPieH, mcMain, objProps, this.plotX, this.plotX + this.plotWidth);
			// method called to set mouse event listener for this Pie3D instance
	...
	private function setEventHandlers(_mc:MovieClip, objProps:Object, Pie3DRef):Void {
		...
		if (this.params.clickURL == '') {
			...
			if (!(this.config.enableRotation || this.config.enableLinks || this.config.totalSlices == 0)) {
				if(this.params.showLegend && this.params.interactiveLegend && this.data[_mc.dataId].label){
					insRef.lgnd.clickEvent(_mc.dataId);
				}else{
					//Enable for clickURL
					this.invokeClickURLFromPlots(_mc);
				}
			} else {
				//Enable for clickURL
				this.invokeClickURLFromPlots(_mc);
		...
		//
		if(this.config.totalSlices > 1){
			//Add listener for "slicing" event from Pie3D objects for slice movement start or end.
			Pie3DInsRef.addEventListener('slicing', this);
		}
	}
	...
	private function sliceViaJS(id:Number):Void{
		...
		if(this.config.totalSlices > 1 && !this.config.changingDimension && !this.rotating && this.chartInit){
			...
			if(this.params.showLegend && this.params.interactiveLegend && this.data[id].label){
				//id = (this.params.reverseLegend) ? this.num - id - 1: id;
				// slice/unslice the pie via legend which takes care of all management issues
				this.lgnd.clickEvent(id);
			// else if legend is not there in the chart	
			}else{
				// get the reference of the Pie3D instance for the pie
				var Pie3DInsRef:Pie3D = this.config.objPie3D[this.map[id]];
	...
	*/
	private function legendClick(target:Object):Void{
		...
		if (this.config.totalSlices > 1 && this.chartDrawn) {
			//
			// slice/unslice the pie
			this.config.objPie3D[this.map[target.index]].movePie();
			//Raise Legend Click external event
			this.raiseLegendItemClickedEvent({index:target.index, name:this.data[target.index].label});
		}else{

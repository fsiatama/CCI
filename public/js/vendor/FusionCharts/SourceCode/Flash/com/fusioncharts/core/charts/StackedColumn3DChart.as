...
class com.fusioncharts.core.charts.StackedColumn3DChart extends SingleYAxis3DChart
{
	...
	*/
	private function parseAttributes (graphElement : XMLNode) : Void
	{
		...
		//Blocked as canvas padding can not be modified in column chart from outside.
		this.params.canvasPadding = 10 //getFN (atts ["canvaspadding"] , 10);
		...
		//If maxColWidth < 0 make absolute value of it
		this.params.maxColWidth = (this.params.maxColWidth < 0) ? Math.abs(this.params.maxColWidth) : this.params.maxColWidth ;
		//If maxColWidth is zero reset  it to 1
		this.params.maxColWidth = (this.params.maxColWidth == 0) ? 1 : this.params.maxColWidth ;
	...
	*/
	private function preparePercentData(): Void{
		...
		if (this.params.stack100Percent){
			...
			for (i = 1; i <= this.numDS; i ++)
			{
				for (j = 1; j <= this.num; j ++)
				{
					if (this.dataset [i].data [j].isDefined){
						...
						percentValue = (this.dataset [i].data [j].value/((this.sums[j].sum == 0)? 1: this.sums[j].sum))*100;
						...
						if (this.dataset[i].data[j].exDispVal==""){
							...
							if (this.params.showPercentValues){
								...
								this.dataset[i].data[j].exDispVal = formatNumber (this.dataset [i].data [j].value, this.params.formatNumber, this.params.decimals, this.params.forceDecimals, this.params.formatNumberScale, this.params.defaultNumberScale, this.config.nsv, this.config.nsu, this.params.numberPrefix, this.params.numberSuffix, this.params.scaleRecursively, this.params.scaleSeparator, this.params.maxScaleRecursion);
							}
						}
						...
						if (this.params.showPercentInToolTip){
							...
							//Store formatted value
							this.dataset[i].data[j].toolTipDispVal = formatNumber (this.dataset [i].data [j].value, this.params.formatNumber, this.params.decimals, this.params.forceDecimals, this.params.formatNumberScale, this.params.defaultNumberScale, this.config.nsv, this.config.nsu, this.params.numberPrefix, this.params.numberSuffix, this.params.scaleRecursively, this.params.scaleSeparator, this.params.maxScaleRecursion);
						}
			...
			for (j = 1; j <= this.num; j ++)
			{
				this.sums [j].displayValue = formatNumber (this.sums [j].sum, this.params.formatNumber, this.params.decimals, this.params.forceDecimals, this.params.formatNumberScale, this.params.defaultNumberScale, this.config.nsv, this.config.nsu, this.params.numberPrefix, this.params.numberSuffix, this.params.scaleRecursively, this.params.scaleSeparator, this.params.maxScaleRecursion);
			}
	...
	private function calculateAxisLimits () : Void
	{
		...
		if (!isNaN(this.params.yAxisMinValue)){
			...
			for (i = 1; i <= this.numDS; i ++)
			{
				...
				//Case 1 - If first dataset's any value is less than the yAxisMinValue.
				//Case 2 - If any other dataset's, except the first, has negative value.
				//Case 3 - If any other dataset's contain lesser negative value than the yAxisMinValue.
				minValueToCompare = (i == 1 || this.params.yAxisMinValue < 0)? this.params.yAxisMinValue : 0;
				for (j = 1; j <= this.num; j ++)
				{
	...
	*/
	private function calculatePoints (isRedraw:Boolean)
	{
		...
		for (i = 1; i <= this.numDS; i ++)
		{
			for (j = 1; j <= this.num; j ++)
			{
				if(this.params.showPercentValues || this.params.showPercentInToolTip){
					//Calculate the % value of the data
					var percentValue = (this.dataset [i].data [j].value / ((this.sums[j].sum == 0)? 1: this.sums[j].sum)) * 100;
				}
				
				//Format and store
				if(this.params.showPercentValues){
					this.dataset [i].data [j].displayValue = formatNumber (percentValue, this.params.formatNumber, this.params.decimals, this.params.forceDecimals, this.params.formatNumberScale, this.params.defaultNumberScale, this.config.nsv, this.config.nsu, "", "%");
				}else{
				...
				if (this.dataset [i].data [j].toolText == undefined || this.dataset [i].data [j].toolText == "")
				{
					...
					if(this.params.stack100Percent){
						toolText = toolText + this.dataset [i].data [j].toolTipDispVal
					}else{
						if(this.params.showPercentInToolTip){
							toolText = toolText + formatNumber (percentValue, this.params.formatNumber, this.params.decimals, this.params.forceDecimals, this.params.formatNumberScale, this.params.defaultNumberScale, this.config.nsv, this.config.nsu, "", "%");
						}else{
							//actual value
							toolText = toolText + formatNumber (this.dataset [i].data [j].value, this.params.formatNumber, this.params.decimals, this.params.forceDecimals, this.params.formatNumberScale, this.params.defaultNumberScale, this.config.nsv, this.config.nsu, this.params.numberPrefix, this.params.numberSuffix, this.params.scaleRecursively, this.params.scaleSeparator, this.params.maxScaleRecursion);
						}
					}
		...
		if (this.params.showSum == true && this.params.stack100Percent==true)
		{
			//Create text field to get height
		...
		if (this.params.labelDisplay == "ROTATE")
		{
			...
			var maxLabelHeight : Number = ((canvasWidth - (2 * this.params.canvasPadding)) / finalVisibleCount);
		...
		//Recalculating  caption and subcaption width so that it takes maximum canvas width 
		//For caption
		if (this.params.caption != "")
		{
			var capHt:Number = this.elements.caption.h;
			//Create element for caption - to store width & height
			this.elements.caption = returnDataAsElement (0, 0, canvasWidth, capHt);
		}
		//For sub-caption
		if (this.params.subCaption != "")
		{
			var subCapHt:Number = this.elements.subCaption.h;
			//Create element for sub caption - to store height
			this.elements.subCaption = returnDataAsElement (0, 0, canvasWidth, subCapHt);
		}
		...
		{
			colWidth = (colWidth > this.params.maxColWidth && this.defaultPlotSpacePercent) ? this.params.maxColWidth : colWidth;
		}
		...
		this.adjustYAxisLabelStep(yAxisValMaxHeight, this.elements.canvasBg.h);
		...
		} else if (this.config.yMax < 0 && this.config.yMin < 0)
		{
			...
			this.config.basePlanePos = this.getAxisPosition (this.config.yMax, this.config.yMax, this.config.yMin, this.elements.canvasBg.y, this.elements.canvasBg.toY, true, 0);
		} else
		{
		...
		labelMetrics.canvasPadding = this.params.canvasPadding + (this.config.columnWidth / 2) + this.config.columnSpace;
		...
		if(this.params.XTLabelManagement){
			...
			//accomodate x shift
			labelMetrics.leftPanelObj.leftPanelMinWidth -= this.config.shifts.xShift;
			labelMetrics.rightPanelObj.rightPanelMinWidth += this.config.shifts.xShift + this.params.canvasBgDepth;
		...
		for (i = 1; i <= this.num; i ++)
		{
			...
			if(this.params.XTLabelManagement){
				this.categories [i].x = this.elements.canvasBg.x + (amountOfDiff/2) + (newSpaceWidth * i) + ((i - 0.5) * newColumnWidth) - this.config.shifts.xShift - this.params.canvasBgDepth/2;
			}else{
			...
			for (j = 1; j <= this.numDS; j ++)
			{
				if (this.dataset [j].data [i].isDefined)
				{
					...
					if (this.config.yMax < 0 && firstDataFound==true && dataValue < 0 ){
						//Add y-max value to calibrate on scale
						dataValue = dataValue + this.config.yMax;
					}
					//Height for each column
					dataEndY = this.getAxisPosition (dataValue, this.config.yMax, this.config.yMin, this.elements.canvasBg.y, this.elements.canvasBg.toY, true, 0);
					...
					//Store value textbox y position applying the valuepadding
					this.dataset [j].data [i].valTBY = this.params.valuePadding*((dataValue >= 0)? -1 : 1 ) + this.dataset [j].data [i].y - (this.dataset [j].data [i].h / 2);
					/**
					  *Adjustment of value textbox y position so that the textbox
					  *never goes outside canvas area due to valuepadding
					**/
					//For positive value
					if(this.dataset [j].data [i].value >= 0){
						if(this.dataset [j].data [i].valTBY  < (this.dataset [j].data [i].y - this.dataset [j].data [i].h) ){
							this.dataset [j].data [i].valTBY = this.dataset [j].data [i].y - this.dataset [j].data [i].h;
						} else if(this.dataset [j].data [i].valTBY  >  this.dataset [j].data [i].y){
							this.dataset [j].data [i].valTBY  = this.dataset [j].data [i].y;
						}
					}else
					//For negetive value
					{
						if(this.dataset [j].data [i].valTBY  > dataEndY){
	...
	*/
	private function drawHeaders ()
	{
		...
		if (this.params.caption != "")
		{
			...
			var xPos:Number = (captionStyleObj.align=="center")?(this.elements.canvasBg.x + (this.elements.canvasBg.w / 2)):((captionStyleObj.align=="left")?(this.elements.canvasBg.x):(this.elements.canvasBg.toX));
		...
		if (this.params.subCaption != "")
		{
			...
			//Now, based on alignment, decide the xPosition of the caption
			var xPos:Number = (subCaptionStyleObj.align=="center")?(this.elements.canvasBg.x + (this.elements.canvasBg.w / 2)):((subCaptionStyleObj.align=="left")?(this.elements.canvasBg.x):(this.elements.canvasBg.toX));
		...
		if (this.params.xAxisName != "")
		{
			...
			var xAxisNameObj : Object = createText (false, this.params.xAxisName, this.cMC, this.dm.getDepth ("XAXISNAME") , this.elements.canvasBase.x + (this.elements.canvasBase.w / 2) , this.height -(this.params.chartBottomMargin + this.config.legendAreaHeight + this.elements.xAxisName.h + this.params.xAxisNamePadding), 0, xAxisNameStyleObj, true, this.elements.xAxisName.w, this.elements.xAxisName.h);
			//Apply animation
			if (this.params.animation)
			{
				this.styleM.applyAnimation (xAxisNameObj.tf, this.objects.XAXISNAME, this.macro, this.elements.canvasBase.x + (this.elements.canvasBase.w / 2) - (this.elements.subCaption.w / 2) , 0, this.height -(this.params.chartBottomMargin + this.config.legendAreaHeight + this.elements.xAxisName.h + this.params.xAxisNamePadding), 0, 100, null, null, null);
			}
			//Apply filters
	...
	*/
	private function drawColumns ()
	{
		...
		var renderedColCount : Number = 0, renderedPColCount:Number=0, renderedNColCount:Number=0;
		...
		for (i = 1; i <= this.num; i ++)
		{
			...
			for (j = 1; j <= this.numDS; j ++)
			{
				...
				if (this.dataset [j].data [i].isDefined)
				{
					...
					colIns = new Column3D (colMC, this.dataset [j].data [i].x, this.dataset [j].data [i].y - this.config.basePlanePos - this.config.shifts.yShift, this.dataset [j].data [i].w, this.dataset [j].data [i].h, this.config.shifts.xShift, this.config.shifts.yShift, this.dataset [j].data [i].color, this.params.showPlotBorder, this.params.plotBorderColor, this.params.plotBorderAlpha);
					//Draw the column
		...
		for (i = 1; i <= this.numDS; i ++)
		{
			for (j = 1; j <= this.num; j ++)
			{
				...
				if (this.dataset [i].data [j].isDefined && this.dataset [i].data [j].showValue)
				{
					...
					//Change the vertical alignment when the label is at the top or bottom edge of the columns
					//For positive value
					if(this.dataset [i].data [j].value >=0){
						if(yPos <= (this.dataset [i].data [j].y - this.dataset [i].data [j].h) + valueStyleObj.size/2){
							vAlign = "bottom";
						}
						else if(yPos >= (this.dataset [i].data [j].y  - valueStyleObj.size/2)){
							vAlign = "top";
						}
					}
					//For negetive value
					else
					{
						...
						if(yPos <= (this.dataset [i].data [j].y + valueStyleObj.size/2)){
							vAlign = "bottom";
						}
						else if(yPos >= (this.dataset [i].data [j].y + this.dataset [i].data [j].h)){
							vAlign = "middle";
						}
		...
		if (this.params.showSum)
		{
			...
			for (i = 1; i <= this.num; i ++)
			{
				...
				if (this.sums [i].sum != 0)
				{
					...
					//Get the y position
					// In case of negetive valupadding label will be on top of the column.
					if(this.params.valuePadding > 0){
						yPos = this.config.basePlanePos + ((this.sums [i].sum < 0) ? (this.sums [i].nY + this.config.shifts.yShift+this.params.valuePadding) : (this.sums [i].pY - this.params.valuePadding));
					}else{
					...
					//Create simulated text object to get height
					var simulatedSumTextObj : Object = createText (true, this.sums [i].displayValue, this.tfTestMC, 1, 0, 0, 90, angle, valueStyleObj);
					/**
					  * Adjust the textbox y position so that it never goes outside of the canvas
					  * due to valuepadding
					**/
					if(yPos <= this.elements.canvasBg.y + simulatedSumTextObj.height/2){
						yPos = this.elements.canvasBg.y ;
						valueStyleObj.vAlign = "bottom";
					}else if(yPos >= this.elements.canvasBg.toY){
						yPos = this.elements.canvasBg.toY;
					}
					//Delete the simulated text object
					delete simulatedSumTextObj;

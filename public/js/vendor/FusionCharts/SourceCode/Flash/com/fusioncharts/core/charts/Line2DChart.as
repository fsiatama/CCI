...
class com.fusioncharts.core.charts.Line2DChart extends SingleYAxis2DVerticalChart 
{
			...
			if (arrDocElement [i].nodeName.toUpperCase () == "GRAPH" || arrDocElement [i].nodeName.toUpperCase () == "CHART")
			{
				...
				for (j = 0; j < arrLevel1Nodes.length; j ++)
				{
					...
					if (arrLevel1Nodes [j].nodeName.toUpperCase () == "SET")
					{
						...
						var setColor : String = formatColor (getFV (atts ["color"] , this.params.lineColor));
						...
						var setValuePosition:String = getFV(atts["valueposition"], this.params.valuePosition);
						...
						this.data [this.num] = this.returnDataAsObject (setName, setValue, setColor, setExDispVal, setAlpha, setToolText, setLink, setShowLabel, setShowValue, setValuePosition, setDashed, setAnchorSides, setAnchorRadius, setAnchorBorderColor, setAnchorBorderThickness, setAnchorBgColor, setAnchorAlpha, setAnchorBgAlpha);
	...
	*/
	private function parseAttributes (graphElement : XMLNode) : Void 
	{
		...
		//Label Display Mode - AUTO, WRAP, STAGGER, ROTATE or NONE
		...
		//Palette color to be used
		var paletteColor:String = this.defColors.getColor ();
		//Line Properties
		this.params.lineColor = formatColor (getFV (atts ["linecolor"] , ((this.params.paletteColors!="")?(paletteColor):(this.defColors.get2DPlotFillColor (this.params.palette)))));
	...
	private function calculatePoints ()
	{
		...
		} else 
		{
			...
			for (i = 1; i <= this.num; i ++)
			{
				...
				if (this.data [i].showLabel)
				{
					//If the label has HTML rendering, push it in the array, so that we can get max height
			...
			if (this.data [i].showLabel)
			{
				...
				if (this.num>1 && count==this.config.finalVisibleCount){					
					...
					if (this.params.labelDisplay == "WRAP"){
						if ((this.data [i].x+labelWidth)>(this.width-this.params.chartRightMargin)){												
							//Allot max available space available between this and previous label.
							labelWidth = (this.data [i].x - this.data [lastShowIndex].x)/2;
							//Set alignment to right
							align = "right";
						}
					}else if (this.params.labelDisplay == "ROTATE"){
						//If in rotate mode, check height
						if ((this.data [i].x+(labelHeight/2))>this.width){												
							//Allot max available space available between this and previous label.
							labelHeight = (this.data [i].x - this.data [lastShowIndex].x);
						}
					}
				}
				...
				if (this.params.labelDisplay == "ROTATE")
				{
					...
					//Create text box and get height
					labelObj = createText (false, this.data [i].label, this.cMC, depth, this.data [i].x, this.elements.canvas.toY + this.params.labelPadding, this.config.labelAngle, labelStyleObj, true, this.config.wrapLabelWidth, labelHeight);
				} else if (this.params.labelDisplay == "WRAP")
				{
					//Case 2 (WRAP)
					//Set alignment
					...
					labelStyleObj.vAlign = "bottom";
					labelObj = createText (false, this.data [i].label, this.cMC, depth, this.data [i].x, this.elements.canvas.toY + this.params.labelPadding, 0, labelStyleObj, true, labelWidth, this.config.wrapLabelHeight);
		...
		if (this.params.drawAnchors)
		{
			...
			for (i = 1; i <= this.num; i ++)
			{
				...
				if (this.data [i].isDefined)
				{
					...
					if(this.data [i].anchorSides < 3){
						//Draw the circle
						DrawingExt.drawCircle(anchorMC, 0, 0, this.data [i].anchorRadius, this.data [i].anchorRadius, 0, 360)
					} else {
						//Draw the polygon
	...
	private function drawValues () : Void 
	{
		...
		for (i = 1; i <= this.num; i ++)
		{
			...
			if (this.data [i].isDefined && this.data [i].showValue)
			{
				...
				if (this.data[i].valuePosition=="AUTO"){
				...
				}else if(this.data[i].valuePosition=="ABOVE"){
					//Above
					vAlign = "top";
					yPos = this.data [i].valTBY - this.params.valuePadding;
				}else{
